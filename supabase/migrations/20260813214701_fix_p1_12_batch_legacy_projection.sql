-- The P1-11 compatibility projection keeps jobs.planned_date/time aligned
-- with the earliest scheduled occurrence. The batch RPC must maintain it the
-- same way the single-occurrence update RPC does.
...
begin
  ...
  -- Maintain the P1-11 legacy projection: the job mirrors its earliest
  -- scheduled occurrence (same rule as update_planning_occurrence).
