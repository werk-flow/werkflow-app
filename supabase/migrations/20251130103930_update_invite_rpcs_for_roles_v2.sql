-- Drop the existing get_invite_by_code function to change its return type
DROP FUNCTION IF EXISTS get_invite_by_code(text);

-- Update redeem_organization_invite to use invited_role from the invite
CREATE OR REPLACE FUNCTION redeem_organization_invite(p_invite_code text)
...
begin
  ...
  -- Get invite details including the email it was sent to and the invited_role
  ...
  -- IMPORTANT: Check that the logged-in user's email matches the invite email
  -- This prevents users from redeeming invites meant for other people
  ...
  -- add membership with the role specified in the invite (defaults to employee)
...
-- Recreate get_invite_by_code with invited_role in the return type
CREATE FUNCTION get_invite_by_code(p_invite_code text)
RETURNS TABLE (
  id uuid,
...
GRANT EXECUTE ON FUNCTION get_invite_by_code(text) TO authenticated;
GRANT EXECUTE ON FUNCTION get_invite_by_code(text) TO anon;