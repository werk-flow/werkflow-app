-- P1-21: NULL is not false in SQL. Reject a missing actor membership
-- explicitly and keep the atomic wrapper's missing-target result distinct.
...
begin
  ...
  if v_actor_role is null or v_target_role is null
    or v_actor_role not in ('admin', 'buero')
...
begin
  ...
  -- Preserve the existing pre-P1-33 destructive removal contract atomically.
