...
create or replace function public.create_planning_entry(
...
begin
  ...
  loop
    ...
    insert into public.planning_occurrences (
      ...
      nullif(v_occurrence->>'originalStartLocal', ''),
      v_job_id,
      (v_occurrence->>'entryKind')::public.planning_entry_kind,
      ...
      (v_occurrence->>'timeKind')::public.planning_time_kind,
      'Europe/Berlin',
    ...
    loop
      insert into public.planning_occurrence_assignments (
        organization_id, occurrence_id, employee_record_id, team_source_id,
        assigned_by
      ) values (
    ...
    insert into public.planning_occurrence_assessments (
      ...
      qualification_snapshot, qualification_fingerprint, override_reason,
      created_by
    ) values (
      p_organization_id, v_occurrence_id,
      coalesce(array(
...
grant execute on function public.create_planning_entry(
  uuid, uuid, jsonb, jsonb, jsonb, jsonb, text, jsonb, text, text
) to service_role;
