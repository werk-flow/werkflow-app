...
- Background `router.refresh()` reconvergence over already-visible content gets no loader at all; the "Ansicht wird aktualisiert…" trailing spinners of the teams and qualifications sections were removed on 2026-08-22 for this reason, not skeletonized.
- Named exception: `job-dispatch-section` renders nothing while loading, by design. The section exists only when dispatch cards exist, so a skeleton would flash and vanish on every job without a dispatch (2026-08-22).

## Feedback

### The vocabulary

- `Banner` (`components/ui/banner.tsx`, shown via the global provider's `useBanner()`): top-center, dismiss X, `role="alert"`. Variants: `success` (green), `error` (red), `info` (blue), `progress` (neutral, spinner or progress bar). Timings are encoded in the component: 3 s auto-dismiss standard, 5 s with an action button ("Rückgängig"), `error` and `progress` persist until resolved or dismissed. Wrappers: `UrlFlashBanner` (post-redirect confirmations via URL param) and `UndoBanner` (success + action). The dismiss button's accessible name is „Hinweis schließen", never plain „Schließen": a locator for „Schließen" collides in strict mode with page action buttons, which cost one full run on 2026-08-21.
...
| Full-page settings form fails | Error banner (a page-level save has no dialog point of action; settings pages keep this deliberately, 2026-08-22) |
