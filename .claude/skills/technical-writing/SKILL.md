...
description: "Layered technical-writing standard: Diátaxis structure, Google developer style sentences, STE instruction rules, Global English syntax. Use when writing, restructuring, or reviewing repo documentation, feature specs, decision records, plans, READMEs, PR descriptions, or commit messages."
---

# Technical writing

Adapted for WerkFlow from cursor/plugins pstack (MIT). Repo notes: this skill covers `docs/`, `AGENTS.md`, skills, and developer-facing prose — all English. German user-facing UI copy is product copy, not documentation; it follows `AGENTS.md`'s language rules and the werkflow-design skill instead. Doc-structure conventions (status headers, index rows, one home per fact, link syntax, `bun run docs:check`) live in `docs/README.md` and are not repeated here.
