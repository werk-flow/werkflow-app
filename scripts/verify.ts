...
import { z } from "zod";
import { calculateBuildInputs, readBuildReceipt } from "../lib/testing/build-identity";
...
function commandForGroup(group: TestGroup, target: "local" | "cloud"): string[] {
  switch (group.kind) {
    ...
    case "static": {
      const script = { "static:typecheck": "typecheck", "static:lint": "lint", "static:docs": "docs:check", "static:coverage": "test:coverage" }[group.id];
      if (!script) throw new Error(`Unknown static group ${group.id}`);
...
async function main(): Promise<void> {
  ...
  const snapshot = captureInputSnapshot(repository, calculateBuildInputs(repository).environmentDigest);
  const history = readHistory().filter((report) => report.target === options.target);
  const priorResults = history.flatMap((report) => report.results);
  ...
  const preliminaryPlan = selected.map((group) => {
    ...
    let reusable = group.kind === "static" || group.kind === "canary" ? undefined : reusableGroupResult({ groupId: group.id, fingerprint, results: priorResults });
  ...
  await withWorkspaceTestLock({ operation: "independent verification groups" }, async () => {
    if (changedInputs(snapshot, captureInputSnapshot(repository, calculateBuildInputs(repository).environmentDigest)).length) throw new Error("Inputs changed after planning. Re-plan before executing or reusing results.");
    ...
    const report: z.infer<typeof reportSchema> = { version: 1, id, startedAt: new Date().toISOString(), completedAt: null, mode: options.mode, target: options.target, scope: options.groupIds ? "selected-groups" : "all-required-groups", jobs: options.jobs, status: "running", snapshot, selected: planning.map((entry) => entry.group.id), results: [] };
    ...
    try {
      ...
      await runGroupSchedule({ entries: planning, jobs: options.jobs,
        ...
        run: async (entry) => {
        if (entry.reusable) { report.results.push(entry.reusable); publish(); return; }
        const startedAt = new Date().toISOString();
        const logPath = resolve(directory, `${entry.group.id.replaceAll(":", "-")}.log`);
        const blocked = globalBlock ?? (entry.group.kind === "static" ? undefined : groupAttemptProblem({ groupId: entry.group.id, fingerprint: entry.fingerprint, results: priorResults, recoveredRunKeys: recoveredEnvironmentRuns(listRunManifests()) }));
        if (blocked) {
        ...
        const current = captureInputSnapshot(repository, calculateBuildInputs(repository).environmentDigest);
        const drift = changedInputs(snapshot, current);
        const latencyProblems = run ? checkLatencyEvidence({ directory: runDirectory(run.runKey), ...entry.timing }).problems : [];
      ...
      if (changedInputs(snapshot, captureInputSnapshot(repository, calculateBuildInputs(repository).environmentDigest)).length) {
