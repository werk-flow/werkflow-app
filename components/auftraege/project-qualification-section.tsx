...
export function ProjectQualificationSection({ projectId }: { projectId: string }): ReactElement {
  ...
  if (loading) return <Card className="flex items-center gap-2 p-4 text-sm text-muted-foreground"><Loader2 className="size-4 animate-spin" />Qualifikationen werden geladen…</Card>
  const byId = new Map(capabilities.map((capability) => [capability.id, capability]))
  return <Card className="gap-4 p-4"><div><h3 className="flex items-center gap-2 text-sm font-semibold"><Award className="size-4" />Geplante Qualifikationen</h3><p className="mt-1 text-xs text-muted-foreground">Projektweite Rollenplanung; noch keine Einsatz- oder Terminentscheidung.</p></div><ErrorText>{loadError}</ErrorText><div className="flex flex-wrap items-end gap-2"><div className="min-w-56 flex-1"><Label>Qualifikation hinzufügen</Label><SearchableSelect options={capabilities.filter((capability) => !requirements.some((item) => item.capability_id === capability.id)).map((capability) => ({ value: capability.id, label: capability.name }))} value={selectedId} onChange={setSelectedId} searchPlaceholder="Qualifikation suchen…" emptyMessage="Keine weitere Qualifikation" disabled={saving || Boolean(loadError)} /></div><label className="flex h-9 items-center gap-2"><Checkbox checked={requireConfirmation} disabled={saving || Boolean(loadError)} onCheckedChange={(checked) => setRequireConfirmation(checked === true)} />Bestätigung nötig</label><Button type="button" variant="outline" disabled={saving || Boolean(loadError) || !selectedId} onClick={() => void save([...requirements.map((item) => ({ capabilityId: item.capability_id, requireConfirmation: item.require_confirmation })), { capabilityId: selectedId, requireConfirmation }]).then((saved) => { if (saved) { setSelectedId(''); setRequireConfirmation(false) } })}>{saving && <Loader2 className="size-4 animate-spin" />}Hinzufügen</Button></div>{requirements.length === 0 && !loadError ? <p className="text-sm text-muted-foreground">Keine Qualifikationen geplant.</p> : requirements.map((requirement) => { const capabilityName = byId.get(requirement.capability_id)?.name ?? 'Unbekannte Qualifikation'; return <div key={requirement.id} className="flex items-center justify-between rounded-md border px-3 py-2"><div><p className="text-sm font-medium">{capabilityName}</p>{requirement.require_confirmation && <p className="text-xs text-muted-foreground">Bestätigung erforderlich</p>}</div><Button type="button" variant="ghost" size="icon" disabled={saving} aria-label={`${capabilityName} entfernen`} onClick={() => void save(requirements.filter((item) => item.id !== requirement.id).map((item) => ({ capabilityId: item.capability_id, requireConfirmation: item.require_confirmation })))}><Trash2 className="size-4" /></Button></div> })}</Card>
}
