  ...
  return (
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div><Label htmlFor="template-search" className="sr-only">Arbeitsvorlagen suchen</Label><Input id="template-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Arbeitsvorlagen suchen…" /></div>
...
function ItemsEditor({ draft, editable, onChange }: { draft: WorkTemplateDraft; editable: boolean; onChange: (draft: WorkTemplateDraft) => void }) {
  ...
  return <section className="space-y-3"><div className="flex items-center justify-between"><div><h3 className="font-semibold">Aufgaben und Checkliste</h3><p className="text-sm text-muted-foreground">Dieselben Einträge erscheinen später in der bestehenden Checkliste.</p></div>{editable && <Button type="button" variant="outline" size="sm" onClick={addItem}><Plus className="size-4" />Eintrag</Button>}</div>
    ...
    {draft.items.map((item, index) => { const evidence = draft.evidence.filter((entry) => entry.templateItemId === item.id); const predecessorIds = draft.dependencies.filter((entry) => entry.dependentItemId === item.id).map((entry) => entry.predecessorItemId); return <Card key={item.id} className="gap-3 py-4"><CardContent className="space-y-3">
      <div className="grid gap-3 sm:grid-cols-[1fr_150px_150px_auto]"><div><Label htmlFor={`item-${item.id}`}>Bezeichnung</Label><Input id={`item-${item.id}`} value={item.content} disabled={!editable} onChange={(event) => patchItem(index, { content: event.target.value })} /></div><div><Label>Art</Label><Select value={item.itemKind} disabled={!editable} onValueChange={(value) => patchItem(index, { itemKind: value as 'task' | 'checklist' })}><SelectTrigger aria-label={`Art für ${item.content || 'Eintrag'}`}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="task">Aufgabe</SelectItem><SelectItem value="checklist">Checkliste</SelectItem></SelectContent></Select></div><div><Label>Verbindlichkeit</Label><Select value={item.requirementState} disabled={!editable} onValueChange={(value) => patchItem(index, { requirementState: value as 'required' | 'optional' })}><SelectTrigger aria-label={`Verbindlichkeit für ${item.content || 'Eintrag'}`}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="required">Erforderlich</SelectItem><SelectItem value="optional">Optional</SelectItem></SelectContent></Select></div>{editable && <div className="flex items-end gap-1"><Button type="button" size="icon" variant="ghost" disabled={index === 0} onClick={() => onChange({ ...draft, items: move(draft.items, index, -1) })} aria-label="Eintrag nach oben"><ArrowUp className="size-4" /></Button><Button type="button" size="icon" variant="ghost" disabled={index === draft.items.length - 1} onClick={() => onChange({ ...draft, items: move(draft.items, index, 1) })} aria-label="Eintrag nach unten"><ArrowDown className="size-4" /></Button><Button type="button" size="icon" variant="ghost" onClick={() => removeItem(item.id)} aria-label="Eintrag löschen"><Trash2 className="size-4" /></Button></div>}</div>
      <div className="grid gap-3 sm:grid-cols-2"><div><Label htmlFor={`group-${item.id}`}>Gruppe</Label><Input id={`group-${item.id}`} value={item.groupLabel ?? ''} disabled={!editable} onChange={(event) => patchItem(index, { groupLabel: event.target.value || null })} placeholder="z. B. Inbetriebnahme" /></div><div><Label>Voraussetzungen</Label><SearchableMultiSelect ariaLabel={`Voraussetzungen für ${item.content || 'Eintrag'}`} options={draft.items.filter((other) => other.id !== item.id).map((other) => ({ value: other.id, label: other.content || 'Unbenannter Eintrag' }))} selectedIds={predecessorIds} onSelectionChange={(ids) => replacePredecessors(item.id, ids)} placeholder="Keine Voraussetzungen" searchPlaceholder="Eintrag suchen…" emptyMessage="Kein anderer Eintrag" disabled={!editable} /></div></div>
      <div><Label htmlFor={`notes-${item.id}`}>Hinweise</Label><Textarea id={`notes-${item.id}`} value={item.notes ?? ''} disabled={!editable} onChange={(event) => patchItem(index, { notes: event.target.value || null })} /></div>
...
function MaterialsEditor({ draft, editable, onChange, inventoryItems, inventoryLocations, onInventoryCreated }: { draft: WorkTemplateDraft; editable: boolean; onChange: (draft: WorkTemplateDraft) => void; inventoryItems: InventoryPickerOption[]; inventoryLocations: InventoryLocation[]; onInventoryCreated: (item: InventoryPickerOption) => void }) {
  ...
  return <section className="space-y-3"><div className="flex items-center justify-between"><div><h3 className="font-semibold">Geplantes Material</h3><p className="text-sm text-muted-foreground">Mengen werden geplant, nicht reserviert oder ausgebucht.</p></div>{editable && <Button type="button" size="sm" variant="outline" onClick={add}><Plus className="size-4" />Material</Button>}</div>{draft.materials.map((line) => <Card key={line.id} className="gap-3 py-4"><CardContent className="grid gap-3 sm:grid-cols-2"><div><Label>Artikel</Label><SelectWithCreate items={inventoryItems} getOption={(item) => ({ value: item.id, label: item.name, description: item.internalSku ?? undefined })} value={line.itemId} onValueChange={(value) => onChange({ ...draft, materials: draft.materials.map((item) => item.id === line.id ? { ...item, itemId: value, isBillable: inventoryItems.find((option) => option.id === value)?.isBillable ?? item.isBillable } : item) })} createLabel="Neuen Artikel erstellen" disabled={!editable} renderCreateDialog={(props) => <CreateMaterialDialog {...props} />} onCreated={onInventoryCreated} /></div><div><Label>Bevorzugtes Lager</Label><LocationSelectWithCreate locations={inventoryLocations} value={line.preferredLocationId ?? ''} onValueChange={(value) => onChange({ ...draft, materials: draft.materials.map((item) => item.id === line.id ? { ...item, preferredLocationId: value || null } : item) })} allowNone disabled={!editable} /></div><div><Label htmlFor={`quantity-${line.id}`}>Geplante Menge</Label><QuantityStepper id={`quantity-${line.id}`} value={String(line.plannedQuantity).replace('.', ',')} min={0.001} step={1} disabled={!editable} onChange={(value) => onChange({ ...draft, materials: draft.materials.map((item) => item.id === line.id ? { ...item, plannedQuantity: parseDecimalInput(value) } : item) })} unitLabel={inventoryItems.find((item) => item.id === line.itemId)?.unit} /></div><div className="flex items-end justify-between gap-3"><label className="flex h-11 items-center gap-2"><Checkbox checked={line.isBillable} disabled={!editable} onCheckedChange={(checked) => onChange({ ...draft, materials: draft.materials.map((item) => item.id === line.id ? { ...item, isBillable: checked === true } : item) })} />Abrechenbar</label>{editable && <Button type="button" size="icon" variant="ghost" onClick={() => onChange({ ...draft, materials: draft.materials.filter((item) => item.id !== line.id) })} aria-label="Material löschen"><Trash2 className="size-4" /></Button>}</div><div className="sm:col-span-2"><Label htmlFor={`material-notes-${line.id}`}>Notiz</Label><Textarea id={`material-notes-${line.id}`} value={line.notes ?? ''} disabled={!editable} onChange={(event) => onChange({ ...draft, materials: draft.materials.map((item) => item.id === line.id ? { ...item, notes: event.target.value || null } : item) })} /></div></CardContent></Card>)}</section>
}
