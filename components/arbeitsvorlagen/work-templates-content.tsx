...
function CreateTemplateDialog({ open, onOpenChange, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; onCreated: (id: string) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><form onSubmit={submit} className="contents"><DialogHeader><DialogTitle>Arbeitsvorlage erstellen</DialogTitle><DialogDescription>Lege zuerst Ziel und Namen fest. Inhalte ergänzt du im nächsten Schritt.</DialogDescription></DialogHeader><div className="space-y-4"><Field label="Name" htmlFor="new-template-name" required><Input autoFocus value={name} onChange={(event) => setName(event.target.value)} /></Field><Field label="Gilt für" htmlFor="new-template-target"><Select value={targetType} onValueChange={(value) => setTargetType(value as WorkTemplateTargetType)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="job">Aufträge</SelectItem><SelectItem value="project">Projekte</SelectItem></SelectContent></Select></Field><Field label="Beschreibung" htmlFor="new-template-description"><Textarea value={description} onChange={(event) => setDescription(event.target.value)} /></Field><ErrorText>{error}</ErrorText></div><DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={isPending}>Abbrechen</Button><Button type="submit" disabled={isPending}>{isPending && <Loader2 className="size-4 animate-spin" />}Erstellen</Button></DialogFooter></form></DialogContent></Dialog>
}
...
function CreateMaterialDialog({ open, onOpenChange, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; onCreated: (item: InventoryPickerOption) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><form onSubmit={submit} className="contents"><DialogHeader><DialogTitle>Artikel erstellen</DialogTitle><DialogDescription>Der Artikel wird ohne Bestand angelegt.</DialogDescription></DialogHeader><div className="space-y-4"><Field label="Name" htmlFor="quick-item-name" required><Input value={name} onChange={(event) => setName(event.target.value)} /></Field><Field label="Einheit" htmlFor="quick-item-unit" required><Input value={unit} onChange={(event) => setUnit(event.target.value)} /></Field><ErrorText>{error}</ErrorText></div><DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={isPending}>Abbrechen</Button><Button type="submit" disabled={isPending}>{isPending && <Loader2 className="size-4 animate-spin" />}Erstellen</Button></DialogFooter></form></DialogContent></Dialog>
}
...
function CreateCapabilityDialog({ open, onOpenChange, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; onCreated: (item: CapabilityDefinition) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><form onSubmit={submit} className="contents"><DialogHeader><DialogTitle>Qualifikation erstellen</DialogTitle><DialogDescription>Erweitert den gemeinsamen Qualifikationskatalog der Organisation.</DialogDescription></DialogHeader><div className="space-y-4"><Field label="Name" htmlFor="quick-capability-name" required><Input value={name} onChange={(event) => setName(event.target.value)} /></Field><Field label="Art" htmlFor="quick-capability-kind"><Select value={kind} onValueChange={(value) => setKind(value as CapabilityKind)}><SelectTrigger aria-label="Art der Qualifikation"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="skill">Fähigkeit</SelectItem><SelectItem value="certification">Zertifizierung</SelectItem></SelectContent></Select></Field><ErrorText>{error}</ErrorText></div><DialogFooter><Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={isPending}>Abbrechen</Button><Button type="submit" disabled={isPending}>{isPending && <Loader2 className="size-4 animate-spin" />}Erstellen</Button></DialogFooter></form></DialogContent></Dialog>
}
