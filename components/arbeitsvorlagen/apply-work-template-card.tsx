...
export function ApplyWorkTemplateCard({
  ...
  const previewPending = Boolean(versionId) && preview === null && error === null
  ...
  function submit(approval?: AssignmentApproval) {
    ...
    void runPendingTask(async () => {
      const result = await applyWorkTemplate({ templateVersionId: versionId, ...(targetType === 'job' ? { jobId: targetId } : { projectId: targetId }), allowAdditional, idempotencyKey: `apply-${targetType}-${targetId}-${versionId}`, assignmentApproval: approval ?? null }).catch(() => null)
      if (!result?.success) {
        if (!result) { setError('Die Arbeitsvorlage konnte nicht angewendet werden.'); return }
