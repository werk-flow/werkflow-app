...
export function ApplyWorkTemplateCard({
  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={(next) => { if (!isPending) setOpen(next) }}><DialogContent className="sm:max-w-xl"><DialogHeader><DialogTitle>Arbeitsvorlage anwenden</DialogTitle><DialogDescription>Die Inhalte werden in diesen {targetType === 'job' ? 'Auftrag' : 'Projekt'} kopiert und bleiben danach unabhängig von der Vorlage bearbeitbar.</DialogDescription></DialogHeader><DialogBody className="space-y-4 py-1"><div className="space-y-2"><Label>Arbeitsvorlage</Label>{options === null ? <Skeleton className="h-9" /> : <SearchableSelect options={options.map((option) => ({ value: option.versionId, label: option.name, description: `Version ${option.versionNumber}` }))} value={versionId} onChange={(value) => { setVersionId(value); setPreview(null); setError(null) }} placeholder="Arbeitsvorlage wählen" searchPlaceholder="Arbeitsvorlagen suchen…" emptyMessage="Keine passende Arbeitsvorlage veröffentlicht" />}</div>{preview && <div className="rounded-lg border bg-muted/20 p-4 text-sm"><p className="font-medium">{preview.name} · Version {preview.versionNumber}</p><p className="mt-1 text-muted-foreground">{preview.itemCount} Aufgaben/Checklistenpunkte · {preview.evidenceCount} Nachweise · {preview.materialCount} Materialpositionen · {preview.capabilityCount} Qualifikationen · {preview.dependencyCount} Abhängigkeiten</p>{preview.hasSameVersionApplication && <p className="mt-3 text-destructive">Diese Version wurde bereits angewendet.</p>}{preview.hasExistingApplication && !preview.hasSameVersionApplication && <label className="mt-3 flex items-start gap-2"><Checkbox checked={allowAdditional} onCheckedChange={(checked) => setAllowAdditional(checked === true)} /><span>Weitere Vorlage ergänzen. Vorhandene Planung bleibt bestehen.</span></label>}</div>}<ErrorText>{error}</ErrorText></DialogBody><DialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)} disabled={isPending}>Abbrechen</Button><Button type="button" onClick={() => submit()} disabled={isPending || !preview || preview.hasSameVersionApplication || (preview.hasExistingApplication && !allowAdditional)}>{isPending && <Loader2 className="size-4 animate-spin" />}Anwenden</Button></DialogFooter></DialogContent></Dialog>
