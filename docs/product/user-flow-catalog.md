...
- `BASE-TIME-F03` — Niemand kann gleichzeitig in zwei Organisationen eingestempelt sein; beim Abmelden wird automatisch ausgestempelt. Das Entfernen eines Mitglieds mit erfasster Arbeitszeit wird abgelehnt und verändert weder seine laufende Sitzung noch seine Zeithistorie (siehe `BASE-ORG-F07`).
...
- `P1-21-F01` — Ein Mitarbeiter startet seine eigene Zeiterfassung über eine eindeutige, touchfreundliche Hauptaktion: Die Stempeluhr-Schaltfläche öffnet eine Liste der nächsten Aktionen („Arbeit starten“, „Arbeit an Auftrag …“, „Fahrt starten“, „Weitere Aktivitäten …“), jede Aktion startet mit einem Tipp; WerkFlow zeigt die laufende Aktivität und den Auftrag in der Anzeige über der Schaltfläche, die während der Arbeit grün und in der Pause gelb mit Tassensymbol erscheint; die verstrichene Zeit zeigt die Uhr auf der Zeiterfassungsseite. Über der Schaltfläche liegen während der Erfassung zwei Schnelltasten für die zwei wahrscheinlichsten nächsten Schritte (geändert in Pre-Wave-3 Schritt 3, 2026-09-15).
...
- `P1-21-F09` — „Pause“ unterbricht die laufende Aktivität, ohne die Anwesenheitssitzung zu beenden; während Arbeit oder Fahrt ist „Pause“ eine Schnelltaste über der Schaltfläche und die erste Aktion der Liste (geändert in Pre-Wave-3 Schritt 3, 2026-09-15).
- `P1-21-F10` — Nach einer Pause setzt der Mitarbeiter bewusst Arbeit oder eine andere Aktivität fort; die Pause wird dabei atomar geschlossen. „Weiter: <Aktivität · Auftrag>“ setzt mit einem Tipp die Aktivität und den Auftrag fort, die die Pause unterbrochen hat; „Weiter ohne Auftrag“ und „Anderer Auftrag …“ bleiben daneben (geändert in Pre-Wave-3 Schritt 3, 2026-09-15).
...
- `P1-21-F19` — Ein Mitarbeiter wählt standardmäßig nur relevante zugewiesene Aufträge; die Auswahl zeigt die für heute eingeplanten Aufträge zuerst unter „Heute geplant“; eine fremde Zuordnung wird bei der Schreibaktion erneut abgewiesen (geändert in Pre-Wave-3 Schritt 3, 2026-09-15).
...
### `P1-24a` — Plantafel und Kalender-Überarbeitung (2026-09-18)
...
- `P1-24a-F02` — Alle: Der Horizont der Plantafel ist 1, 2, 4 oder 6 Wochen (Auswahl in der Kopfzeile); „Zurück“ und „Weiter“ springen um den Horizont, „Diese Woche“ kehrt zur aktuellen Woche zurück, und der heutige Tag ist in der Kopfzeile markiert.
...
- `P1-24a-F04` — Alle: Spalten sind Berliner Kalendertage. Wochenenden sind schattiert und per Filter „Wochenende ausblenden“ ausblendbar; Feiertage und Betriebsruhe schattieren die Spalte und tragen ihre Bezeichnung im Kopf; arbeitsfreie Tage einer Person sind in ihrer Zeile schattiert, Tage vor Eintritt oder nach Austritt schraffiert.
- `P1-24a-F05` — Alle: Eine Karte ist ein Termin: Uhrzeit, Titel, Kunde und Ort, Auftragsnummer; ganztägige und mehrtägige Termine sind ein Balken über ihre Tage; Serientermine tragen das Serienzeichen; ausgelassene und abgesagte Termine sind gedämpft, durchgestrichen und nicht verschiebbar.
- `P1-24a-F06` — Alle: Jede Karte eines Auftragsbesuchs zeigt den Einsatzstatus der Person als Chip: nicht gesendet, gesendet, bestätigt, übernommen, Rückfrage, nicht möglich.
- `P1-24a-F07` — Büro/Admin: Karten zeigen den Bereitschafts-Chip „Material nicht reserviert“, sobald für den Auftrag Material geplant ist, mit derselben Formulierung wie das Einsätze-Panel; ein Werkzeug-Chip erscheint nicht, weil der Kalender dazu keine Tatsache kennt.
- `P1-24a-F08` — Alle: Abwesenheiten liegen als Balken in der Zeile: „Urlaub – Name“, angefragter Urlaub gestrichelt mit „(angefragt)“, Krankheit neutral als „Abwesend – Name“; Handwerker sehen nur eigene Abwesenheiten.
- `P1-24a-F09` — Büro/Admin: Jede Person-Tag-Zelle zeigt ihre Auslastung (frei, teilweise geplant, voll, überbucht) als stille Zahl „geplant / Soll“ mit farbiger Markierung bei voll und überbucht; der vollständige Satz erscheint beim Überfahren und als Vorlesetext der Zelle.
- `P1-24a-F10` — Büro/Admin: Eine Karte in eine andere Zeile ziehen besetzt den Termin um, mit denselben Dialogen wie bisher (Qualifikation, Kapazität, Einsatz-Ablösung, Kundenzusage). In eine andere Spalte ziehen verschiebt das Datum; Uhrzeit und Dauer bleiben. Ein Ziel auf einem Urlaubstag, einem arbeitsfreien Tag oder außerhalb der Beschäftigung wird schon beim Ziehen mit Satz abgelehnt; mit gedrückter Umschalttaste ist die Ablage trotzdem möglich und öffnet den Hinweis-Dialog.
- `P1-24a-F11` — Büro/Admin: Der rechte oder linke Rand eines ganztägigen Balkens verlängert oder verkürzt ihn tageweise.
- `P1-24a-F12` — Büro/Admin: Eine Karte auf den Parkplatz-Schalter oder das Parkplatz-Panel ziehen parkt den Auftrag: die Karte verschwindet sofort und der Kontext-Dialog öffnet sich; Abbrechen stellt die Karte wieder her. Interne Termine lassen sich nicht parken; die Meldung sagt das.
- `P1-24a-F13` — Büro/Admin: Eine Parkplatz-Karte auf eine Zelle ziehen plant den Auftrag bei dieser Person an diesem Tag ein, in einem Serveraufruf mit Rückabwicklung bei Fehlschlag; im Tag landet sie auf einer Uhrzeit. Auf dem Desktop steht der Parkplatz neben der Plantafel und verdeckt keine Spalte; Escape schließt ihn. Ein geparkter Auftrag ohne Parkplatz-Kontext (Altbestand) wird schon beim Ziehen mit Satz abgelehnt und bietet kein „Einplanen am …“, bis ein Kontext ergänzt ist.
...
- `P1-24a-F18` — Büro/Admin: „Nur ansehen“ in der Kopfzeile sperrt jedes Ziehen in allen drei Ansichten; ein Zugversuch zeigt den Grund; die Einstellung bleibt je Benutzer erhalten.
...
- `P1-24a-F20` — Alle: Tastenkürzel `t` heute, `j` und `k` weiter und zurück, `d`, `w`, `m` Ansicht, `z` macht die letzte Ablage rückgängig solange der Hinweis sichtbar ist, `+` und `-` zoomen die Tagesansicht, `?` zeigt die Liste; in Eingabefeldern und offenen Dialogen passiert nichts.
- `P1-24a-F21` — Alle: Ansicht, Horizont, Dichte („Kompakt“ und „Komfortabel“), Wochenende, Filter, Suche, „Nur ansehen“ und „Ist-Zeiten anzeigen“ bleiben je Benutzer und Organisation erhalten und gelten beim nächsten Öffnen; der Kalender öffnet in der zuletzt gewählten Ansicht, sonst Büro und Admin auf der Plantafel und Handwerker im Tag.
- `P1-24a-F22` — Alle: „Ist-Zeiten anzeigen“ blendet je Person-Tag die erfasste Ist-Zeit als „Ist“-Marke ein, provisorische Korrekturen in der Wartefarbe; bei einer Woche standardmäßig an, bei längeren Horizonten aus.
...
- `P1-24a-F24` — Handwerker: sehen nur die eigene Zeile, eigene Termine, eigene Abwesenheiten und Ist-Zeiten; keine Kapazität, Abwesenheit oder Termine anderer; kein Ziehen. Mitglieder anderer Organisationen erhalten für die Plantafel-Daten keine Antwort.
...
- `P1-24a-F26` — Alle: Die Tagesansicht zeigt Zeilen mit inhaltsabhängiger Höhe; überlappende Blöcke liegen untereinander in Bahnen; das Raster rastet auf 15 Minuten (mit Umschalttaste 5); Griffe zum Verlängern sind 8 px breit mit 24 px Trefffläche; arbeitsfreie Tage und Abwesenheiten der Person sind schattiert; kurze Lücken zwischen Terminen tragen ihre Minuten; die Stundenachse beginnt bei 05:00 im Bild und zoomt mit Strg und Mausrad.
- `P1-24a-F27` — Büro/Admin: Ziehen auf leerer Fläche einer Zeile öffnet den Eintragsdialog mit Person, Beginn und Ende vorbelegt; ungeplante Aufträge des Tages liegen in einer Ablage je Zeile und lassen sich auf das Raster ziehen; das „+“ am Namen öffnet den Dialog per Tastatur.
- `P1-24a-F28` — Alle: Die Jetzt-Linie ist in Tag, Plantafel (heutige Spalte) und Monat (heutiger Tag) dieselbe Linie mit Punkt in derselben Farbe.
- `P1-24a-F29` — Büro/Admin: Ist-Zeit-Blöcke lassen sich im Tag verschieben, verlängern, verkürzen und auf eine andere Person hängen, im Monat auf einen anderen Tag; Überlappung mit einem anderen Block der Zielperson und ein Zeitpunkt in der Zukunft werden schon beim Ziehen rot mit Satz abgelehnt; eine eigene Änderung eines Büro-Mitglieds bleibt ein Änderungsantrag.
- `P1-24a-F30` — Alle: Die Monatsansicht füllt die Fläche; Zellen wachsen mit ihrem Inhalt; „+n mehr“ öffnet die Tagesliste an Ort und Stelle; vergangene Tage sind gedämpft; Feiertage und Betriebsruhe stehen am Tag, Abwesenheiten und mehrtägige Termine sind Balken über ihre Tage; Doppelklick oder die Tageszahl öffnet den Tag.
- `P1-24a-F31` — Büro/Admin: Im Monat verschiebt Ziehen einen Besuch auf einen anderen Tag mit denselben Dialogen; die Karte landet sofort und springt nur bei einer Ablehnung zurück.
...
- `P1-24a-F33` — Büro/Admin: Parkplatz und Einsätze sind Seitenpanels auf derselben Ebene; Parkplatz-Karten sind Zugquellen und bieten „Einplanen am …“ als Formular mit Datum, Uhrzeit und Person.
- `P1-24a-F34` — Alle: Bei 1280 px Breite scrollt keine Ansicht seitlich (Plantafel bei einer Woche, Monat); Tag und längere Horizonte scrollen nur innerhalb des Kalenders; bei 2560 px wachsen Spalten und Karten; auf dem Telefon zeigt die Woche eine Tagesliste, nie ein beschnittenes Raster: die eigene Woche für Handwerker, für Büro und Admin die Termine aller Personen mit Namen je Karte.
- `P1-24a-F35` — Alle: Jeder Zustand (heute, jetzt, Abwesenheit, Feiertag, Betriebsruhe, provisorische Zeit, Ziehen, Ziel, Ablehnung) ist im hellen und dunklen Modus lesbar; Text in Karten ist nie kleiner als 11 px.

**Acceptance invariant:** `35/35 mapped; 35/35 fully evidenced; 0 partial; 0 unmapped` (accepted 2026-09-24).
