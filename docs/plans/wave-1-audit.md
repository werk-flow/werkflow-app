...
7. **Final confirmation runs — always LAST, after all review fixes.** Fresh production build; if the session changed app code, **one full golden suite run** (93 tests) green per `testing.md`'s numbered rules — once per session, not per fix; and the focused `@AUDIT-W1-A<N>` run green against the production build as the session's acceptance evidence. Nothing may change after these runs — any later code or test change reopens step 5 and requires a fresh confirmation (the P1-09 lesson).
8. **Document and publish.** Update every ledger row for the session (status + evidence), append a session entry to the log below, update the catalog where corrected, commit on local `main`, publish with `git push origin main:partner-preview`. Never plain `git push`, never `origin main`.
