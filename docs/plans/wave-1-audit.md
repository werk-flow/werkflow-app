...
5. **Regression-prove app-code fixes.** If (and only if) the session changed app code: run the focused golden spec(s) of the affected slice(s), then **one full golden suite run** (93 tests) on a fresh production build per `testing.md`'s rules — once per session, not per fix. Sessions that only added test code skip the full golden run but still finish with their focused audit spec green on a production build.
6. **CodeRabbit review** (per `docs/technical/coderabbit.md`): required for any session whose diff contains app-code changes; sessions with test/doc-only diffs may batch their review with the next such session (record the deferral in the ledger). Disposition findings as usual.
7. **Document and publish.** Update every ledger row for the session (status + evidence), append a session entry to the log below, update the catalog where corrected, commit on local `main`, publish with `git push origin main:partner-preview`. Never plain `git push`, never `origin main`.
