...
2. **`GG-00` coverage gaps.** Automated v2 (11/11 passing on a production build) now covers time tracking, Realtime freshness, and mobile viewport in addition to v1. Still not automated: invites/onboarding flows and inventory take/return. Either extend the gate or record an owner-run manual pass for those two areas before marking `P1-00` complete. The v2 cycle also found and fixed two real defects: missing Realtime refresh on `/kunden`, and a pre-hydration login fallback that put credentials into the URL.
