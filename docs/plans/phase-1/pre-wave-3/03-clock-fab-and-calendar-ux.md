...
- Tests repointed: the five clock helpers in `tests/golden/support/steps.ts` plus `openActivityDialogFromSheet`, `clockInConfirmationButton`, the P1-21 golden and audit specs; the dialog-contract, route-inventory, read-request-scope and duplicate-helper inventories carry the new dialog, route and modules.
...
4. Test helpers: one helper per flow in `tests/golden/support/steps.ts`, updated names in the P1-21, A1 and canary specs, the `clock-readiness` contract extended (readiness, resume job, hot keys disabled until ready).
