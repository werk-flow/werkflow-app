...
| --- | --- | --- | --- |
| Tier 1 | Database authorization derives the actor from `auth.uid()`. Client-callable policies cannot trust a supplied user ID. Raw helpers that accept an actor ID have no `authenticated` grant. | `20260902114500_close_p1_24_effective_review_findings.sql` | `supabase/tests/p1_24_people_lifecycle.sql` checks grants, Admin/Büro/employee roles, and outsider denial. |
| Tier 1 | Every cached membership and lifecycle read keeps `organization_id` in the predicate. A stale organization cookie falls back only when the cookie is absent, not when access is blocked. | `lib/data/cached.ts`, `lib/auth/redirects.ts`, `lib/personnel/lifecycle-actions.ts` | P1-24 Golden suspension, rule-12 isolation, A1 membership checks, and the SQL tenant matrix. |
| Tier 1 | Protected classification is independent of caller-visible rows. Protected files cannot enter `ordinary_documents`, ordinary links, folders, move, copy, trash, or permanent-delete paths. | `app_private.p1_24_is_protected_document` in `20260902133000_bind_p1_24_document_classification.sql`; `lib/documents/actions.ts` | Checked SQL covers classification, link/delete guards, version/history RLS, and Büro restricted-file denial. Golden proves absence from `/dokumente`. |
| Tier 1 | Employee access binds one active release to the exact released `document_versions.version_number`. A later replacement is not exposed by an older release. Release changes lock the document before reading its current version. | `public.set_personnel_document_release` in migration `20260902114500`; `getAuthorizedDocument` in `lib/documents/actions.ts` | Checked SQL tests release, replacement, revocation, version read, audit read, and replay. Golden records one exact receipt. |
| Tier 1 | Lifecycle writes use one operation schema, exact organization and expected-version guards, serialized replay lookup, and append-only events. A repeated operation returns its first result; conflicting payloads fail. | `lib/personnel/lifecycle.ts`; migrations `20260902085933`, `20260902092940`, and `20260902114500` | Lifecycle units and checked SQL cover stale versions, replay order, conflicts, immutable history, and a unique operation identity. |
| Tier 1 | A due activation cannot bypass an unresolved access blocker. Cancelling a blocked scheduled activation records the correction. Immediate suspension remains available. Last-Admin and owner safety stays in the database transition function. | `20260902050010_enforce_p1_24_access_readiness.sql`; `20260902114500_close_p1_24_effective_review_findings.sql` | Checked SQL covers blocker refusal, due-transition cancellation, suspension, reactivation, and last-Admin safety. |
| Tier 1 | Personnel storage paths sanitize traversal characters before length truncation. Cleanup tickets expire after 45 minutes, use `getSupabaseSecretKey()`, and log cleanup failure without hiding the original upload error. | `lib/documents/storage-path.ts`, `lib/personnel/lifecycle-actions.ts`, `lib/documents/upload-client.ts` | `lib/personnel/lifecycle.test.ts` pins bounded paths. P1-24 Golden covers the direct browser-to-R2 ticket, finalize, and recovery path. |
| Tier 1 | Own-action queries exclude deleted documents and expose only the affected employee's active release or requested evidence. Responsibility ownership never grants file access. | `lib/personnel/lifecycle-actions.ts`; protected-document RLS in migrations `20260902042104`, `20260902114500`, and `20260902133000` | Checked SQL role matrix plus Golden employee/outsider reads and affected A4 privacy proof. |
| Tier 1 | Requirement resolution is explicit. Manager actions record fulfilled, waived, or cancelled state with a reason instead of hiding a blocker through presentation state. | `components/mitarbeiter/personnel-lifecycle-section.tsx`, `lib/personnel/lifecycle-actions.ts`, requirement RPCs | Rule-12 audit, checked SQL, and Golden activation after acknowledgement. |
| Tier 2 | The shared searchable picker waits for a visible and enabled trigger before bounded remount retries. | `tests/golden/support/steps/` (one file per domain since 2026-09-25) | Final A3 5/5 and A1 28/28. |
| Tier 2 | The A3 role test distinguishes access to the combined approvals page from authority over the time-approval panel. | `tests/audit/wave-1/a3-personal.spec.ts` | Final A3 5/5. |
| Tier 2 | Every persisted P1-24 stage reloads its stable personnel ID, asserts its predecessor state, and waits for dialog closure before a database assertion. | `tests/golden/p1-24.spec.ts` | Focused P1-24 4/4, expanded GG-07 8/8, and complete Golden 142/142. |
| Tier 2 | Schema drift, unsafe grants, RLS gaps, replay errors, teardown errors, zero-row rollout, and Realtime replica identity fail checked commands. | `supabase/tests/p1_24_people_lifecycle.sql`, `package.json`, generated types, and `lib/realtime/tables.ts` | `test:sql:p124`, `types:check`, `migrations:check`, and `realtime:check` passed at closure. |
