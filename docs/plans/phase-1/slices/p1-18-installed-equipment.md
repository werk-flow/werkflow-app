...
A fresh-agent preflight then exposed one reporting ambiguity: `test:runs list` showed 17 historical `failed_retained` results without showing that all 17 worlds had already been cleaned. The command now prints `open`, `cleaned` or `none` for every displayed row, lists every open run key even when it falls outside the last 30 rows, and ends with `Open retained worlds: N`. Focused unit tests pin the distinction and the full-archive count (Tier 2). The live report labeled all 17 historical failures `cleaned` and ended with `Open retained worlds: 0`. The complete unit suite passed 382/382 across 36 files with 707 expectations; TypeScript, ESLint, `docs:check` and `git diff --check` also passed. No additional cleanup or browser run was required.
...
Implement the confirmed P1-18 slice (the contract sections above) across the additive Supabase model, TypeScript domain boundary, manager service routes, compact customer-site projection, explicit assigned-job field projection, existing document owner, Realtime invalidation and staged acceptance tests.
