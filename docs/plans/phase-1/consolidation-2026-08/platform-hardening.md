...
| D4 | Latency contract v1: a user's own action reflects instantly (optimistic echo); another session's open surface shows the change within **2 seconds** target; the test helper hard-fails above **5 seconds** on the local stack and records the measured time. Numbers are provisional until real local-stack measurements exist. Recalibrated to 15 s, see resolution 5 under the Stage B resolutions. |
...
| D6 | **Per-stage closure duties (owner requirement):** before a stage was closed, the stage agent (1) swept the documentation set for contradictions with the stage's outcome and reconciled them — [testing.md](../../../technical/testing.md), [realtime-and-caching.md](../../../technical/realtime-and-caching.md), [environments.md](../../../technical/environments.md), [protocol.md](../protocol.md) wording, the skills, and `AGENTS.md` where affected; (2) re-evaluated every enforcement-ladder rule the stage touched: retired rules the stage made obsolete, converted rules the stage made mechanizable, and added the stage's own new Tier 1/2/3 rules so the stage could not regress; updated [enforcement-ladder-backlog.md](../../../technical/enforcement-ladder-backlog.md) accordingly; (3) updated this file's ledger and decisions; (4) drafted the next stage's handoff prompt; (5) ran `bun run docs:check`, committed, and published via `git push origin main:partner-preview`. |
| D7 | Context handoff model: **this file was the canonical context**, not the prompts. Every stage agent read it fully (all three stages — an agent had to know the arc to avoid doing a later stage's work early). Prompts were thin: they verified the starting position, pointed here, and named the stage's deliverables and its closure duties. The Stage A prompt was drafted at the end of the design session; each stage's agent drafted the next stage's prompt at closure (the proven P1-16 → P1-17 meta-prompt pattern), folding in what that stage had learned. |
...
| C | Measured scenario budgets | done 2026-08-29 | Playwright defaults now use the measured target budgets without per-test overrides: 180 s for local Golden and audit scenarios, 300 s for cloud Golden and audit scenarios, and 300 s for canary scenarios. Superseded 2026-08-31: the audit local budget is 240 s, see [testing.md](../../../technical/testing.md) |
...
## Historical stage prompts (do not execute)

The two prompts below are records of past sessions, kept verbatim. They contain commit and publish instructions and name a next slice that has since been accepted; do not run them.

### Stage B handoff prompt

Drafted at Stage A closure per D7. The owner pasted it verbatim into a fresh session (Fable 5, high reasoning).
...
1. docs/plans/phase-1/consolidation-2026-08/platform-hardening.md IN FULL — the canonical plan: decisions D1–D12, all three stages, the D6 closure duties, the ledger. You are Stage B only.
...
Drafted at Stage B closure per D7. The owner pasted it verbatim into a fresh session (Fable 5, high reasoning).
...
1. docs/plans/phase-1/consolidation-2026-08/platform-hardening.md IN FULL — the canonical plan. You are Stage C, the last stage; your closure also closes the phase.
