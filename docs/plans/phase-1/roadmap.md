...
Status: living — last reviewed 2026-09-06; the hot Phase 1 entry file, always read it fully
...
- `slices/` — one document per started slice: its plan during implementation and its acceptance record after closure.
...
| Field | Current value |
| --- | --- |
| Active slice | None; all Wave 2 slices are accepted, ending with `P1-24` on 2026-09-02 |
| Current work | Beta-client handoff preparation. [Step 1 security and infrastructure hardening](../security-infrastructure-hardening-2026-09.md) closed on 2026-09-07: 28 findings dispositioned, three migrations and both edge functions deployed to DEV and PROD, member removal with time history blocked, Vercel Pro and PITR deferred to the first customer with checklists in the recovery runbook. Its browser evidence is deliberately hand-picked under the cross-cutting-pass rule in [testing](../../technical/testing.md#choose-the-acceptance-scope); the complete release-mode battery runs once at final beta acceptance after the performance pass. The [testing system restructure](../testing-system-restructure-2026-09.md) is implemented and replaces the old full-battery-per-slice workflow. Application acceptance remains blocked by cross-session customer updates above the two-second deadline; its closure record owns the evidence. The complete change and release plans have not passed. General speed optimization follows separately and must preserve the invariants in [security.md](../../technical/security.md). |
| Task / branch owner | Codex on local `main`, publishing only to `partner-preview` |
| Slice gate | None (no active slice) |
| Next slice | `P1-25`, inventory catalog and supplier master data, after the owner resumes slice work following handoff preparation |
...
| Latest completed golden gate | `GG-07` was extended and accepted with P1-24. The 2026-09-04 full Golden run also passed every implemented gate; run evidence is in [golden-gate-log.md](../golden-gate-log.md). |
| Latest hardening proof | September 6 follow-up: expanded phone-layout audit and recorded-production-server enforcement, with qualified static, unit, component, and layout evidence. The [testing restructure closure](../testing-system-restructure-2026-09.md) owns run identities, candidate limits, and the remaining application failures. Earlier mixed-candidate evidence remains in the [September 5 closure](../uiux-and-test-reliability-2026-09.md). |
| Open certification record | The formal Wave 2 end gate remains unrecorded. The UI/UX closure supplies later local battery evidence with an explicit fingerprint limitation, but does not record the complete wave-end requirements. See [wave-2-audit.md](../wave-2-audit.md#wave-end-certification-gate). |
| Production release | Deferred until the owner explicitly requests it after handoff preparation. Continue local `main` and preview-only publication under `AGENTS.md`; this checkpoint does not authorize a commit, push, or production deployment. |
| Last roadmap review | 6 September 2026 |
...
| `P1-33` | `planned` | Employee offboarding completes outstanding time/leave, work/approval reassignment, access revocation, asset/tool/vehicle return, exports, and retained historical identity | `P1-23`, `P1-24`, `P1-32` | Employee management; inventory; time; jobs; documents | Planned/immediate/reversed exit, later correction, asset exception, last-admin, and history tests pass. **Known baseline defect this slice MUST fix** (Wave 1 audit `A1-07`, owner-deferred here 2026-08-15): destructive member removal used to auto-clock the person out and then **hard-delete their entire organization time-entry history**. Contained on 2026-09-06 by the [security hardening pass](../security-infrastructure-hardening-2026-09.md) (SI-006): the database now refuses removal of any member with recorded time, so history can no longer be lost, but such members can only leave through the P1-24 employment transitions. P1-33 replaces this with retained (or explicitly anonymized) history per its retained-historical-identity outcome; its history tests must cover the removal path |
