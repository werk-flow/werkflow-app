...
| 6 | minor | `tests/golden/support/steps.ts` | Disproved | Requiring the warning whenever `overrideReason` is set would break callers that pass a reason for a warning the product does not guarantee (the first team creation in P1-11, recorded under PF-14). The strict check applies only to measured submissions. |
