...
- Asking whether the way we test makes sense is not optional and not a reflection at the end: the second time a run fails for a reason that is not the product, the runner refuses the next verification until a harness input changed (`campaignGateProblem`, [testing.md](../../technical/testing.md#the-campaign-budget)), and the agent changes the harness before that run.
