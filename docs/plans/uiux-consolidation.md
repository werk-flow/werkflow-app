...
| | |
| --- | --- |
| Done | Brief + two full inventories (2026-08-21); canon confirmed; **S1** (canon), **S2** (primitives, 93/93), **M1** (Anfragen + Kunden, 93/93), **M2** (Auftraege + Aufgaben, 93/93), **M3** (Kalender + Dispatch, 93/93), **M4** (Mitarbeiter + Settings + Organization, 93/93) |
| Next | **M5** — Inventar + Dokumente + Zeiterfassung/clock |
| Then | Final gate (full golden + full Wave-1 battery on one frozen build) |
...
### M2 — Auftraege + Aufgaben — **done 2026-08-21**
...
### M3 — Kalender + Dispatch — **done 2026-08-22**
...
### M4 — Mitarbeiter + Settings + Organization — **done 2026-08-22**
...
| 2026-08-21 | **M2**: Auftraege + Aufgaben fully migrated — create/edit job and project dialogs on the canon (close-then-banner, `DialogBody` sections, `ErrorText`, flex chains through the tabbed hosts incl. `calendar-entry-dialog`'s job tab), all listed silent failures fixed plus three found in passing (partial job-link failures in both project forms, `job-detail`'s silent project-save close), park confirmation on `AlertDialogAction` (async via `event.preventDefault()`), capability picker searchable (Label now targets `#job-qualification-capability`), `/aufgaben` loading + org-switch skeletons, sonner removed from auftraege/aufgaben. **Two product defects fixed via the ladder:** the pre-existing deletion stall (DELETE-204-but-no-navigation evidence; all three post-delete redirects now hard-navigate via `window.location.assign`; root-cause task for the broader server-action-stall family remains open) and the `entry-details-dialog` draft wipe (reset effect now keyed on open — the A1-30 flake class). CodeRabbit (3 findings, all applied): try/catch/finally in both actions menus, edit-job submit gated on `isLoadingAssignments`/load failure (empty-crew wipe hazard), singular/plural banner wording. Validation: focused golden 31/31, `@AUDIT-W1-A1` **28/28**, `@AUDIT-W1-A5` **4/4**, post-fix focused 19/19, full golden **93/93** (28.2m, world `mt3apnmq`). **What later sessions inherit:** (1) close any open multi-select popover via its trigger (or by open-state: `button[role="combobox"][aria-expanded="true"]`) before clicking a pinned `DialogFooter` — Playwright never dispatches the dismissing outside click; (2) the environment can stall server-action POSTs (deletes, assignment saves, even setup logins) — evidence and history in `golden-gate-log.md`; a dev-project service restart plus post-reboot clock skew each cost setup attempts; classify before hardening tests; (3) `job-detail-content`'s manual suspend flag stays deliberately — it also guards mutation-in-flight windows (delete navigation, project reassignment) that the automatic dialog convention cannot cover; (4) M3 note: `calendar-entry-dialog` already carries the DialogBody flex chain for its job/project tabs; its planning and entry tabs still scroll whole-dialog until M3 migrates them. | done |
| 2026-08-22 | **M3**: Kalender + Dispatch fully migrated — `planning-entry-form` is a real `<form>` with `DialogBody` and a pinned submit (DatePicker/TimeInput/DurationHoursInput/QuantityStepper replace every native date/time/number input; loading text → skeleton with `role="status"`); `planning-occurrence-edit-dialog` got the same inputs plus the `DialogBody` layout it never had, as a real form whose Enter submits the edit or, in status mode, the status save; `calendar-entry-dialog` now has one Enter behavior and one feedback channel across all three tabs (planning tab carries the flex chain; the entry tab closes immediately with a success banner — `ManualEntryFormContent`'s internals stay M5's, its inline flash simply never renders in this host anymore); `parking-context-dialog` Verantwortlich → `SearchableSelect` with `allowNone`, real form, success banner; `dispatch-panel` day shift → `QuantityStepper` (min −366), panel spinner → section skeleton, `CommitmentDialog`/`ReasonDialog` are real forms on `ErrorText`; `job-dispatch-section` challenge dialog is a real form and its errors use `ErrorText`; the blue `day-view/undo-banner.tsx` (`ActionBanner`) is **deleted** — day-view, calendar-container, and `job-instruction-items-card` now show green undo/error banners through `useBanner()` adapters; the kalender cluster is sonner-free; one ASCII-umlaut message fixed in `calendar-container`; stale lint leftovers in `entry-details-dialog` removed. **Deliberate deviation:** `job-dispatch-section` still renders nothing while loading — the section only exists when dispatch cards exist, so a skeleton would flash-and-vanish on every job without a dispatch (the inventory's `:248` pointer was the conforming in-button spinner, not a section spinner). CodeRabbit: 5 findings, 4 applied — a failed banner-undo now settles the calendar's in-flight refresh counter (`handleSilentRefresh`/`silentRefresh` in the catch — a real suppressed-refresh hazard), skeletons announce loading (`role="status"` + sr-only text), the challenge dialog clears its error on Abbrechen, and the edit dialog's status save is no longer blocked by schedule-edit validations; 1 skipped: `SearchableMultiSelect` exposes no id/aria-label prop to associate its `Label`s (trigger text is the accessible name) — primitive parity gap left for a later session. Validation: focused golden 24/24 (`@GG-00|@P1-11|@GG-03`), `@AUDIT-W1-A6` 7/7, `@AUDIT-W1-A7` 9/9, post-fix focused 11/11 + A6 7/7 + A7 9/9, full golden **93/93** (28.9m, world `mt3ic670`; first full attempt lost only the long-green GG-02 vacation-overlap test to a Realtime re-render click race — dates typed, dialog idle, no submit ran — classified transient per testing rule 10, rerun green unchanged). **What later sessions inherit:** (1) `QuantityStepper` and `DurationHoursInput` keep the element id on their inner text input, so `.fill()` keeps working in specs — the typed-segment helpers are only needed for DatePicker/TimeInput; (2) planning/parking steps (`createPlannedCalendarEntry`, `editPlannedCalendarOccurrence`, `setParkingContextFromParkplatz`) and the a6/a7 inline locators are on the shared helpers; (3) the environment intermittently rejects `sb_secret`-authenticated requests with „JWT issued at future" (gateway-minted JWT vs. node clock skew; killed global setup twice on 2026-08-21) — the leftover sweep in `tests/golden/support/seed.ts` now retries this bounded; if it ever hits app traffic it surfaces as a normal request error; (4) M5 note: the zeiterfassung host of `ManualEntryFormContent` (`manual-entry-dialog.tsx`) still uses the old inline-flash convention, and its double-error surface is unchanged — both are M5 scope. | done |
| 2026-08-22 | **M4**: Mitarbeiter + Settings + Organization fully migrated — qualification management (employee/capability pickers → `SearchableSelect`, three dates → `DatePicker`, warning days → `QuantityStepper`, all toasts → `ErrorText`/banners); team management (per-team member picker searchable, membership dates → `DatePicker`, dissolve error inline in its `AlertDialog`); the delegation dialog is a real form with searchable holder/substitute pickers and `ErrorText` failures; the 17-option Bundesland picker is searchable; the break-rule minute fields dropped `type="number"` (zod keeps the bounds); the sidebar organization switcher is a `SearchableSelect`; work-schedule, employment-condition, create-personnel, and all five sickness dialogs carry the `DialogBody` layout, `ErrorText`, and close-then-banner success; invite cancel/delete now hold their `AlertDialog` open until the server confirms (`event.preventDefault()`) with `ErrorText` and success banners — the listed silent-failure fix. **Banner consolidation done:** `settings-banner-provider` (with its 3 s error auto-dismiss) and dead `settings-feedback-banner` deleted, all ten consumers on the global `useBanner()`; `role-change-banner`, the four dashboard URL banners, and the onboarding duplicate deleted in favor of `UrlFlashBanner` (which gained an `info` variant for „bereits Teil dieser Organisation"; the old banners' org-match latching is deliberately dropped — a 3 s flash does not need it); role changes confirm via the global banner. `SearchableSelect`/`SearchableMultiSelect` gained an `ariaLabel` prop (closes M3's skipped accessible-name finding). **Loading:** `/qualifikationen` streams behind Suspense with a `loading.tsx` and page skeleton; the async settings layout was restructured (static `SettingsShell`, access guard streams behind Suspense) so all 11 `/einstellungen/*` pages render the shell instantly, plus a segment `loading.tsx`; org-switch overlay entries added for `/anfragen`, `/qualifikationen`, and `/einstellungen` (the M4-decided answer to the „skips einstellungen by design" question: cover it). **Deliberate deviations:** the teams/qualifications sections' „Ansicht wird aktualisiert…" trailing spinners were removed rather than skeletonized — they marked background `router.refresh()` reconvergence over already-visible content, where the canon wants no loader; the settings pages keep error banners (not inline `ErrorText`) for full-page form failures — the dialog rule's point-of-action reading for page-level saves. CodeRabbit: **0 findings** (38 files). **Two harness defects surfaced by the batteries, both fixed:** (1) the A5 mega-journey (six catalog creations, five person records, a five-requirement job) overran its 180 s budget once the registry inputs type dates as segments — verified progressing to its final assertions at timeout; now `test.slow()`; (2) A4-R03's schedule guard was weekday-blind — on a Saturday the labeled default source yields a positive target that deliberately consumes no vacation day, so the approved-days snapshot was honestly empty; the guard now uses `doesDateConsumeVacation` per the testing-doc P1-04 precedent (R04's target-based guard is correct for sickness and deliberately unchanged). Validation: focused golden `@GG-00\|@P1-03\|@P1-04\|@P1-05\|@P1-08\|@P1-09` **48/48** (11.4m), `@AUDIT-W1-A3` 5/5, post-fix `@AUDIT-W1-A4` **6/6**, post-fix `@AUDIT-W1-A5` **4/4**, full golden **93/93** (29.6m, world `mt3xvlet`, first attempt). **What M5 inherits:** (1) `ariaLabel` exists on both searchable selects — use it wherever several identical pickers render on one page; (2) team/qualification/delegation/holiday steps and the a5 inline team interaction are on the shared helpers; `#capability-warning-days` and other `QuantityStepper` ids still accept `.fill()`; (3) `UrlFlashBanner` accepts `variant="info"`; (4) every remaining sonner import is M5's (app/layout mount, `vacation-approvals`, dokumente) — removing the dependency and escalating the S1 ESLint warns to error closes the effort; (5) the settings shell no longer wraps a provider — new settings surfaces just call `useBanner()`. | done |
