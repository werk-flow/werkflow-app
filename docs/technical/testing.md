...
Use semantic accessible roles and names where their wording is part of the requirement. A stable identifier can locate a durable business action while a separate assertion verifies its accessible name. Do not accept a list of vaguely similar button labels or use positional selection to make an ambiguous test pass. The `Banner` dismiss button is named „Hinweis schließen"; a locator for plain „Schließen" collides in strict mode with page action buttons and cost one full run on 2026-08-21.

Registered controls have their own step conventions. Close an open multi-select popover (`button[role="combobox"][aria-expanded="true"]`) before clicking a pinned `DialogFooter`, because Playwright never dispatches the outside click that would dismiss it. `QuantityStepper` and `DurationHoursInput` keep the element id on their inner text input, so `.fill()` works on them; the typed-segment helpers (`typeIntoDatePicker`, `typeIntoTimeInput` and their by-id variants in `tests/golden/support/steps.ts`) are only for `DatePicker` and `TimeInput`.
...
Measured failures bypass preparation retries immediately. A vanished dialog does not permit restarting a failed measurement. Outside a measurement, a step treats "the dialog or popover vanished under me" as a bounded, retryable preparation condition, never as something to wait out: dialog suspension drops a scheduled refresh timer but cannot cancel an in-flight `router.refresh()`, so a refresh fired just before the dialog opened can still remount it ([freshness rule 6](realtime-and-caching.md#client-freshness-contract)). `@FRESHNESS` and `@READINESS` identify the measured stages for focused diagnostics; the group registry separately pins required measurements and exclusive scheduling.
...
The larger comparison profile (50 employees, 200 daily occurrences, 10,000 customers, 20,000 historical jobs, three years of history) is an owner-adjustable planning assumption from the Step 2 plan and has never been generated. It exceeds the 2,000-occurrence window cap: 200 occurrences on every day of the 44-day window are 8,800 rows, so a bounded complete-read strategy or an explicit supported-capacity error must be defined before it is measured, and an intentional overflow error must not be classified as slow rendering. Daily density in both profiles means every day, weekends included, as the typical profile's 40 per day already does.
