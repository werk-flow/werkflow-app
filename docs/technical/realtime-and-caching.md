...
Review note, 2026-09-05: the current [Supabase Postgres Changes documentation](https://supabase.com/docs/guides/realtime/postgres-changes#delete-events) says DELETE filtering requires FULL identity and that DELETE delivery does not apply RLS. That conflicts with this repository's former claim that its minimal index identity proves server-side organization-filtered DELETE delivery. The parity checker proves database configuration only. The infrastructure audit must test payloads and cross-organization delivery on the deployed transport before claiming isolation or changing replica identity. Existing same-organization freshness tests do not settle this question.
