...
**Latest parity checkpoint (P1-21, 2026-09-01):** eighteen migrations from `20260831100920` through `20260901012500` were applied DEV-first and then identically to production. Their guarded DEV and PROD ledger keys match the committed filenames. Fresh DEV type generation agrees with the committed canonical-time contract. Production retained 577 legacy time entries, 2 change requests, 23 memberships, 25 personnel records and the one pre-existing open legacy sequence; all four canonical time tables received zero rows. The first remote advisor pass found one P1-21 private trigger without a pinned `search_path`; the forward corrective migration reached local, DEV and PROD and both security advisors now contain only the two pre-existing `set_job_assignment_organization` notices. P1-21 has zero unindexed-foreign-key notice.

**Latest Realtime parity checkpoint (P1-21, 2026-09-01):** `bun run realtime:check` and read-only cloud inspection agree across local, DEV, and PROD. Each backend publishes 80 tables: 77 use replica identity `USING INDEX` on exactly `(id, organization_id)`, 3 use the recorded DEFAULT identity, and 0 use FULL. P1-21 adds only mutable `time_sessions` and `time_segments`; append-only events and operation receipts are not published. The `supabase_realtime` publication has INSERT, UPDATE, and DELETE enabled on all three backends.
