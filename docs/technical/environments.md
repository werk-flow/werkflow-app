...
The owner confirmed on 2026-09-08 that Vercel Preview sets `NEXT_PUBLIC_SUPABASE_URL=https://jbgaqpdjauzoocplgdsn.supabase.co`. `partner-preview` and the production app therefore share the production Supabase project today. A preview deployment isolates application code, not its database. This confirmation covers the Supabase URL; it does not establish every Vercel variable or R2 scope. [Decision 0008](../decisions/0008-development-workflow.md) ends this: before the step 5 preview the owner scopes every Supabase and R2 variable per Vercel environment, Preview to DEV and Production to PROD, so previews show synthetic data and can never touch a customer's. Update this paragraph when that is done.
