...
Status: living — last reviewed 2026-09-14
...
The owner confirmed on 2026-09-08 that Vercel Preview sets `NEXT_PUBLIC_SUPABASE_URL=https://jbgaqpdjauzoocplgdsn.supabase.co`. `partner-preview` and the production app therefore share the production Supabase project. A preview deployment isolates application code, not its database. This confirmation covers the Supabase URL; it does not establish every Vercel variable or R2 scope.
...
- **Production release**: advance `origin/main` only on an explicit release request. A future beta-handoff release is planned after the post-Wave-2 work; it is not authorized by the documentation audit. Preview currently uses production Supabase as confirmed above. Recheck environment scopes and branch overrides when deployment configuration changes, and coordinate shared-database migrations with both deployed app versions.
