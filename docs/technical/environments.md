...
**Latest parity checkpoint (P1-20, 2026-08-31):** fourteen migrations from `20260830210602` through `20260831090000` were applied DEV-first and then identically to production. Their DEV and PROD ledger keys match the committed filenames. Fresh DEV type generation agrees with the committed maintenance contract. The P1-20 security scope is clean; the two production security notices remain the pre-existing `set_job_assignment_organization` grants. A rollout advisor pass found six composite maintenance foreign keys whose existing query indexes used another leading-column order; `20260831090000_cover_p1_20_root_foreign_keys.sql` reached DEV then PROD and removed all six notices. Production retained 5 organizations, 13 customers, 1 site, 40 jobs, 14 projects, 43 documents and 23 document links and received zero rows across all ten maintenance tables, maintenance-owned document links or follow-ups.

**Latest Realtime parity checkpoint (P1-20, 2026-08-31):** `bun run realtime:check` and read-only cloud inspection agree across local, DEV, and PROD. Each backend publishes 78 tables: 75 use replica identity `USING INDEX` on exactly `(id, organization_id)`, 3 use the recorded DEFAULT identity, and 0 use FULL. The three mutable maintenance roots are published; immutable revisions, links and event history are not. The `supabase_realtime` publication has INSERT, UPDATE, and DELETE enabled on all three backends.
