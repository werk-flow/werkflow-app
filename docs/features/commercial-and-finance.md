...
The tax and legal questions that need qualified review, written on 2026-09-17 so the owner can work them through with Willert Haustechnik's in-house experts and their `Steuerberater` before the slices start (protocol caution 2: `P1-39` to `P1-43` stay `decision_blocked` until they are answered). Each item names the slice, the question in plain words, and what WerkFlow needs back; an answered item is replaced by its answer in the list below the table. E5 to E10 and E14 to E16 are tax law for the `Steuerberater`; E1 to E4, E11 to E13 and E17 to E20 are office practice that Willert's own staff can answer, E18 with one email to their vendor; E19 and E20 set parser fixtures and the acceptance journeys' starting case, never scope.
...
### Research facts for Wave 4 (2026-09-17)

Dated facts the Wave 4 slices start from; refresh prices and versions before a contract. The full digest with sources is in [pre-Wave-3 step 4](../plans/phase-1/pre-wave-3/04-wave-3-4-and-phase-2-planning.md#research-digest-2026-09-17).

- Timeline (BMF FAQ, 2026-03-23): receive e-invoices since 2025-01-01; issue from 2027-01-01 above 800,000 € prior-year turnover, from 2028-01-01 for everyone; `Kleinbetragsrechnungen` up to 250 € and B2C exempt; accepted formats XRechnung and ZUGFeRD from 2.0.1 except MINIMUM and BASIC WL.
- Versions: XRechnung 3.0.2 since 2024-06-20, in force until at least 2027-07-31, 4.0 prerelease in September 2026 on EN 16931-1:2026; KoSIT validator configuration 2026-08-31 for Validator 1.6.3; ZUGFeRD 2.5.2 (Factur-X 1.09.2) since 2026-09-01, EN 16931 profile for the ordinary case; XRechnung 3.0 is Germany's Peppol ruleset since 2025-02-01. No B2B Peppol mandate; a managed access point costs about 0,10 to 0,25 € per invoice.
- Libraries: `@e-invoice-eu/core` 3.3.0 (UBL and CII, every profile, TypeScript, WTFPL, JSON-schema validation only); `@stackforge-eu/factur-x` 1.4.1 (CII, PDF/A-3b embedding, EUPL); `node-zugferd` is a stale beta; Mustangproject 2.26.0 (Java, Apache 2.0, validator with VeraPDF, CLI and server); KoSIT validator (Java 11+, Apache 2.0, HTTP daemon). PDF/A: Chromium output is not PDF/A; Gotenberg converts HTML to PDF/A-3b and embeds Factur-X; Typst 0.15 exports PDF/A-3 with `attach` but no ZUGFeRD XMP.
- Invoice rules: §14 Abs. 4 UStG fields; reverse-charge wording "Steuerschuldnerschaft des Leistungsempfängers" with tax category AE; `Bauabzugsteuer` 15 % withheld by the customer unless a `Freistellungsbescheinigung` (§48b, EIBE portal) exists, 5,000 € threshold; Skonto as `#SKONTO#TAGE=14#PROZENT=2.00#` in BT-20; number ranges may have gaps and several ranges (UStAE 14.5 Abs. 10); eight-year retention for invoices from 2025. The EN 16931 mapping of cumulative `Abschlagsrechnungen` and the netting `Schlussrechnung` (BG-3, BT-113) is unconfirmed and goes to E5.
- Accounting: the DATEV-Format `EXTF` Buchungsstapel needs no partner status; the DATEV Rechnungsdatenservice 1.0 and Buchungsdatenservice need Marktplatz partner status (a Datenservice in use by 25 customers, three references, monthly fee plus click price, numbers unpublished); Lexware Office public API (free, XL plan), sevDesk API (all plans), BuchhaltungsButler API, Agenda imports the DATEV file.
- Banking: finAPI (BaFin AISP, FinTS fallback, Access B2X from 100 € per month up to 200 users, 30-day trial); GoCardless Bank Account Data closed to new sign-ups; Enable Banking and Tink on request; consent renews every 180 days (Regulation (EU) 2022/2360); camt.053 v08 replaces MT940 since 2025-11-23. Embedded payments in the market: HERO Wallet on Swan, ToolTime Pay on Adyen (1 % capped at 10 €), Stripe 1.5 % plus 0,25 € cards and 0,35 € SEPA.
- Market bar: plancraft, openHandwerk, TAIFUN and pds claim alternative and demand positions, Aufmaß tied to positions, cumulative `Abschlagsrechnungen` with a netting `Schlussrechnung`, `Sicherheitseinbehalt`, §13b, `Bauabzugsteuer`, Skonto, multi-level dunning and post-calculation; HERO and ToolTime lean on embedded payments; ToolTime sells DATEV Rechnungsdatenservice 1.0 as a 19 € per month add-on.
