...
- **Parked context (P1-12):** a parked job can carry manager-owned context — a bounded reason vocabulary plus note, a responsible office person, and a next-review date with overdue attention — recorded in `job_parking_contexts` with append-only `job_parking_events`. Parked jobs without recorded intent (including all pre-P1-12 rows) stay visible as the labeled „Kontext fehlt (Altbestand)" exception; unparking clears the context traceably. This vocabulary is deliberately provisional until `P1-14` owns the mature status/blocker machine.
