...
Each connector names ownership, direction, credential, retry, deduplication, revocation and fallback before it ships. The decided starting points: inbound email on a WerkFlow address per organization (Amazon SES in `eu-central-1` or Mailgun EU); a customer mailbox connection later through Microsoft Graph (free publisher verification) before Google (annual CASA assessment); calendar as an ICS subscription first, then Microsoft Graph; SMS through seven.io (German provider, 0,075 € per SMS, a phone-number sender so customers can reply; alphanumeric senders are send-only in Germany); WhatsApp last, through the Meta Cloud API via a BSP such as 360dialog with `data_localization_region` DE (template approval, opt-in, per-message charges from October 2026); phone intake, if built rather than partnered, through call forwarding to one WerkFlow-owned national number on Twilio with Ireland routing or sipgate plus an EU realtime voice API, because per-customer German numbers need a business register document and an address inside the prefix each time. Accounting and wholesaler connectors are Wave 4 and Wave 3 scope (`P1-43`, `P1-34`, `P1-50`); an agent never places a supplier order or issues an invoice.
...
Tooling candidates from the 2026-09-17 research, pending the owner's decision: observability and evaluation through Langfuse Cloud in the EU (Ireland; MIT-licensed, self-hosting possible; AI SDK integration) plus promptfoo for offline evaluation, with prompt and trace retention under WerkFlow's own rules; retrieval in Postgres with pgvector 0.8 (`halfvec(1024)`, HNSW, iterative scans) combined with the documented `tsvector` search through reciprocal rank fusion, organization-scoped by the same RLS rows and with no separate vector store at SHK scale; embeddings from Cohere Embed v4 on Bedrock Frankfurt or Mistral EU.
...
The chain of events: Phase 1 continues in its wave order (`P1-24a`, Wave 3, Wave 4, then the pilot lane's two slices under this spec's rules, Wave 5, Wave 6 with `P1-53` and `P1-54`). Phase 2 is then planned the way Phase 1 was: a `docs/plans/phase-2/` roadmap with waves, bounded slices, gates and records, built from three inputs: this spec (the plan above and the considerations list), the "Phase 2 — Intelligence And Automation" section of every feature spec (each names its useful intelligence and approved actions), and the capability map's five levels. Beta feedback reorders slices inside a wave; it does not skip a wave's prerequisites. The pilot lane is the one Phase 2 delivery inside Phase 1, and its two slices are the first records under `docs/plans/phase-2/`.

Prerequisites the Phase 2 planning must settle first, none of them a feature:

- The contract chain: WerkFlow is its customers' processor; adding a model provider, a speech provider and a document-intelligence provider adds sub-processors, which means an updated `Auftragsverarbeitungsvertrag` with each customer, a published sub-processor list in German, and a `Datenschutz-Folgenabschätzung` before the first customer document reaches a model. This is legal work with a lead time and belongs to the offer and onboarding, not to a slice.
- The execution identity: an agent acts under an identity that is neither the user nor the service role. The membership and responsibility model needs a service principal with its own audit attribution; `P1-53` owns it, and decision 0001's auth re-evaluation (before mobile sessions and external identities) is the moment to design it.
- The evaluation set: real, consented SHK documents and dictations (from the beta customer, anonymized where possible) for German offer prose, delivery notes and site notes; without it the model choice is a guess.
- Observability: an error and trace pipeline for the app (today: Vercel runtime errors and Supabase logs) that the AI run ledger and the EU trace store join, so a failed run is an incident with an owner, not a silent retry.
- The product name of the agent, decided before any user-facing copy exists.
- The usage counter and the fair-use line in the offer, so the first pilot slice ships with the counter visible from day one.
