...
- Append-only `vacation_request_events` audits request, decision, withdrawal, and cancellation facts with actor and payload (including the approval snapshot and restored days). SELECT RLS is self-or-manager through the `app_private` SECURITY DEFINER helpers; all writes are service-role server actions with organization-validation triggers; `vacation_requests` is Realtime-published with minimal `USING INDEX` replica identity.

Shared attention pattern (`P1-07`, 2026-08-07):

- One role-aware surface at `/aufgaben` (**Aufgaben**) unifies what previously lived only in scattered queues: pending time sessions and change requests, pending vacation requests, and open client requests appear as derived attention items for exactly the people who can act on them right now — `time_approval`/`leave_approval` holders (including delegated substitutes, whose items appear with the delegation window and disappear when it ends) and managers for requests. Items deep-link into the owning surface; decisions still run exclusively through the owning actions (`reviewSession`, `decideVacationRequest`, …) — the pattern routes attention and never becomes a second way to mutate domain state.
- Affected employees get in-app decision notifications (**Benachrichtigungen**): one per request, deduplicated — a later cancellation makes the same notification unread again instead of adding a second one. Read markers are strictly personal (`attention_read_states`), audited append-only (`attention_events`), and quiet by default: no reminders, escalation, or external delivery (`P1-46`).
- `/aufgaben` also shows **Meine Anträge** — the employee-transparency list of own vacation requests with their decision states and reasons.
- The badge pipeline is unified: the Aufgaben sidebar badge counts actionable items plus unread notifications for the viewer, the Zeiterfassung badge and Anträge tab badge count time **and** vacation approvals (resolving the P1-06 undercount), and a badge never counts an item its viewer cannot act on.

Sickness and privacy-sensitive absence (`P1-08`, 2026-08-08):

- A sickness report is a REPORTED FACT, not a request: `sickness_reports` (keyed to `employee_record_id`) carries `reported` → `cancelled` (recorded in error, reason required when someone else cancels) and nothing else — no approval lifecycle. Everything in between is a traceable correction on the same row (end date set/changed, dates/portion/type corrected), appended to `sickness_report_events` with actor and before/after. Open-ended reports (`end_date` null, „bis auf Weiteres") are first-class, as is retroactive entry; a gist exclusion constraint race-safely blocks overlapping own ACTIVE sickness — deliberately not against vacation, because sickness during approved vacation is a real case.
- Neutral operational absence-type vocabulary only: `krankheit` („Krankheit"), `kind_krank` („Kind krank"), `sonstige` („Sonstige Abwesenheit"). No legal meaning, no entitlement math, and **no field anywhere that could hold a diagnosis** — the domain deliberately has no free-text note (owner decision, 2026-08-08; deferred to the `P1-24` privacy tiers if ever).
- Who records: the person themselves (self-service „Krank melden" in the `/zeiterfassung` **Krankmeldung** section — the 6:45-with-a-phone path) and admin/Büro on the person's behalf via the member/personnel detail **Krankmeldungen** section (the 7:00 phone-call-in path, the common SHK case). The person may set/correct their own end date and cancel their own report; corrections/cancellations of someone else's report are manager-role actions with a required reason. Authority is the fixed manager role resolved server-side at action time — deliberately NOT `leave_approval` (reusing a decision responsibility would leak health-adjacent visibility to selected non-manager employees) and no new responsibility-vocabulary entry (deferred until real demand).
- **The privacy matrix (layered disclosure, the slice's defining deliverable):** the FACT of unavailability (existence, dates, portion) flows wherever availability must flow — targets, calendar, planning. The TYPE and evidence state live only on the person's own surfaces and the admin/Büro management surface. The shared calendar shows neutral „Abwesend – Name" (never „Krank", never the type; open-ended entries say „bis auf Weiteres"); non-managers see only their own entries; colleagues see nothing at all, not even existence. Enforced at three layers: self-or-manager SELECT RLS through the `app_private` SECURITY DEFINER helpers, derivation-time audience scoping in the attention resolver, and type-free calendar payloads.
- Evidence is tracked STATE without file bytes (owner decision, 2026-08-08): per-report `evidence_required` (an explicit per-report office choice presented neutrally — no encoded deadline rules, nothing presented as legal advice) and `evidence_status` `not_required`/`pending` („Nachweis ausstehend")/`received` („Nachweis erhalten"), manager-maintained and audited. Under the current document RLS an uploaded AU certificate would be visible to every admin/Büro member and invisible to the person it concerns, so storing evidence files is deferred to the `P1-24` personnel-document privacy tiers.
- Availability effects arrive through the ONE absence mechanism: the `ApprovedAbsenceSpan` union gained `type: 'sickness'` and both target loaders merge approved vacation with active sickness spans (open ends clamped to the query window; a day covered by both keeps the vacation attribution deterministically). Full day → target 0, half day (single-day reports) → half base target, holiday/closure zeroing stays first. The person's own dashboard says „Krankmeldung – heute keine Sollarbeitszeit."; hour-based absence remains deferred.
- Clocking in on a sick day WARNS instead of blocking (unlike approved vacation): a recovered person showing up early is reality, so clock-in succeeds with a visible notice („Für heute liegt eine Krankmeldung vor …") nudging the end-date correction, and the office sees the contradiction on its surface.
...
- Visibility is layered: employees get a read-only `/qualifikationen` view of only their own teams and qualification records; admin/Büro see and maintain the organization workspace and a targeted summary on each personnel detail; colleagues see nothing. The apprentice-warning switch is admin-only and defaults off. All operational tables have organization-validation triggers, self-or-manager or manager-only RLS as appropriate, Realtime publication, minimal `USING INDEX` replica identity, and typed cache invalidation; audit tables stay append-only and unpublished.
