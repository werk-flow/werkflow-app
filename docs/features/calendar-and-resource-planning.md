...
- A calendar preference (view, horizon, density, filters, „Nur ansehen“) is stored in the user's organization preferences with the change; only the search text waits 600 ms after the last keystroke. A failed save keeps the local value for the session only.
- A job parked before the Parkplatz context existed (P1-12) shows „Kontext fehlt (Altbestand)“ in the Parkplatz and plans only after a manager adds its context: a drag onto the board is refused at the pointer with that sentence, and „Einplanen am …“ is not offered.
