...
As of 2026-09-02, `/kalender` is the shared planning surface for Admin and Büro: they schedule one-off and recurring job visits and internal entries, see capacity and qualification warnings, dispatch work, keep parked work in the `Parkplatz`, and record customer commitments separately from the internal plan. Employees see only the occurrences assigned to them and confirm or challenge their dispatch. Actual working time appears in the same calendar but stays structurally separate from planned work. The week view becomes the `Plantafel` in [`P1-24a`](../plans/phase-1/slices/p1-24a-plantafel.md), the next slice, which the owner extended on 2026-09-18 to a rebuild of the day and month views and the panels on one rendering layer with optimistic drops in every view and a specific German message for every refusal; the [audit in that record](../plans/phase-1/slices/p1-24a-plantafel.md#audit-of-2026-09-18) lists the current defects (doubled and clipped borders and a header scrollbar in the month view, an off-pixel and clipped now dot in the day view, bordered cell cards and a vivid violet today column in the week view, non-optimistic park and month drops, generic refusal messages, React commits per pointer move) with their code locations.
...
- Views and direct manipulation (`P1-24a`, 2026-09-18). The week tab is the **Plantafel** for Admin and Büro: one row per active employee grouped by team, one column per Berlin date over a horizon of one to six weeks, timed visits as cards with the dispatch chip and the material chip, all-day and multi-day visits as bars, absences and closure days in the row, and the planned-versus-target minutes of every person-day. Employees see the same tab as „Woche“ with their own row only. On a phone every role sees the week as a day list, one section per date, with the person's name on each card when more than one row is visible; the phone list carries no toolbar and no drag. The day view keeps people rows against the hour axis with content-driven lanes, an untimed tray, drag-to-create and 15-minute snapping; the month grid grows with its content and opens overflow in a „+n mehr“ popover. All three views share one in-house drag engine (pointer events, one ghost moved by `transform`, no React commit per pointer move), one optimistic owner (`components/kalender/mutations/use-calendar-mutations.ts`: the card is at its target before the first server call, a refusal rolls back with the rule's sentence, success offers Undo through the inverse write) and one typed German message layer (`lib/calendar/messages.ts`). Every drag has a keyboard and form path: the card popover offers „Termin bearbeiten“, „Verschieben …“ and „Parken“, the Parkplatz card „Einplanen am …“. Managers create jobs and entries from the calendar, move, resize and reassign them, park by dropping on the Parkplatz and unpark by dropping a Parkplatz card (on a desktop the Parkplatz panel sits beside the board and narrows it, so every column stays a drop target; on a phone it covers the list; Escape closes it); manual time entries can be created from the calendar. Client pre-checks mirror the server rules (absence, non-working day, employment, inactive occurrence, past midnight, overlapping or future recorded time) and refuse at the pointer; Shift lets a planner take the warning dialog instead.
- Past and started visits (`P1-11`, surfaced 2026-09-25). A timed occurrence is history from its start instant and an all-day one from its Berlin date; the database refuses any rewrite (`started_planning_occurrence_immutable`). The views apply the same rule before anyone tries: the card is not a drag source and carries a lock icon, the popover offers neither „Termin bearbeiten“ nor „Verschieben …“ and names the rule instead, and a resize handle never appears. There is no read-only switch; the owner removed „Nur ansehen“ on 2026-09-25 because a per-user lock that changed nothing visible had no use.
- Drag details (`P1-24a`, 2026-09-25). A drag starts after a 250 ms press or a small movement; the board and the day scroll under a drag only after the pointer has stayed 150 ms in the 40 px edge zone, and a pointer outside the scroll container (over the Parkplatz panel or the header) never scrolls it, so a card carried in from the panel lands where the planner sees it. Escape during a drag cancels the drag; Escape otherwise closes the Parkplatz panel when no dialog is open. A drop on an absence or a non-working day is refused at the pointer and allowed with Shift through the confirmation dialog; a drop outside the person's employment stays refused. „Zurück“ and „Weiter“ move by the horizon and the header's reset reads „Diese Woche“ for one week and „Aktueller Zeitraum“ for longer horizons. The day view's zoom runs from 0.5 to 3 with the keys and to 4 with Ctrl and the wheel; its rows list „Ohne Zuweisung“ first, then the members in the board's team-and-name order, then members without a board row that day. The „Arbeitszeiten“ toggle in the tab row is session state; „Termine“ beside it and every other control are persisted preferences. An empty person-day cell offers one quick action, „Eintrag am … anlegen“, which opens the create dialog for that person and day; `c` opens it for the shown day from anywhere in the calendar, and the board grid is reachable by Tab (the first cell) and the arrow keys.
- Windows, loading, and freshness (Step 2, 2026-09-08; `P1-24a`). Each view reads one window: the selected day plus the previous day, the Monday of the board week minus one day through the end of the horizon plus one day, or the month grid plus one day on each side. Six datasets arrive per window (entries with provisional corrections, occurrences, vacation, sickness, holiday context, and the board context of rows, daily targets, dispatch states and material demand) through two private GETs that commit together. Switching to dates the loaded window already covers shows them at once; an uncovered window keeps the grid visible but inert while it loads, marks a failed refresh as stale, and replaces an uncovered failure with a retry. Per-user calendar preferences (view, horizon, density, weekends, filters, search, read-only, actual time) live under the `calendar` key of the organization user preferences and decide the landing view: the saved view, else the Plantafel for managers and the day for employees.
- Absence and holiday context. The board, the day and the month show public holidays and **Betriebsruhe** as labeled, non-interactive context, approved vacation as „Urlaub – Name“, pending requests dashed with the suffix „(angefragt)“, and sickness as the neutral „Abwesend – Name“ (half days and open ends carry their suffix). Sickness type and evidence never reach the calendar. A person's non-working days are shaded in their row and the days before entry or after exit are hatched.
...
- A calendar preference (view, horizon, density, filters) is stored in the user's organization preferences with the change; only the search text waits 600 ms after the last keystroke. A failed save keeps the local value for the session only.
- A job parked before the Parkplatz context existed (P1-12) shows „Kontext fehlt (Altbestand)“ in the Parkplatz and plans only after a manager adds its context: a drag onto the board is refused at the pointer with „Der Auftrag hat noch keinen Parkplatz-Kontext. Ergänze ihn im Parkplatz über „Kontext ergänzen“, dann lässt er sich einplanen.“, and „Einplanen am …“ is not offered. While the contexts are still loading the pre-check passes; a drop then shows „Der Parkplatz wurde inzwischen geändert. Bitte lade die Ansicht neu.“ and re-reads the contexts and the parked jobs.
- The day view lists the members with a login (and „Ohne Zuweisung“ for managers); a visit assigned to a personnel record without login shows under „Ohne Zuweisung“ there, while the Plantafel shows it in that person's row. A day row per employee record is an open follow-up of `P1-24a`.
...
- The day view shades whole non-working days and absences of a person; it does not shade the hours outside a person's schedule, because the board read carries daily targets, not shift hours (`P1-24a` decision).
- The board's readiness chip covers planned material only („Material nicht reserviert“ when `job_material_lines` exist); a tools chip is not shown because no tool assessment fact exists yet (Wave 3).
...
- Which additional supervision rules are needed beyond the qualification coverage and optional apprentice warning delivered by `P1-09`?
- How should `P1-26` and `P1-32` connect material and tool availability to calendar readiness while keeping reservation and custody with inventory?
...
- The ten open calendar-overhaul decisions of 2026-09-18 (slice shape, month rewrite, the now indicator's colour, density modes, the board at laptop width, the drag engine, month drag, a weekend toggle, day-view row heights, copy by drag) were decided by the owner on 2026-09-18 as recommended and are recorded in the [`P1-24a` record](../plans/phase-1/slices/p1-24a-plantafel.md#product-decisions-required-before-coding): one slice, the month view rebuilt without FullCalendar, a red-family now indicator, two densities, seven days without horizontal scroll at 1280 px, an in-house pointer-event drag engine, month drag, a weekend toggle, content-driven day rows, copy by `Alt`-drag.
