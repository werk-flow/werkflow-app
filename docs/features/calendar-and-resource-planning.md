...
- A calendar preference (view, horizon, density, filters, „Nur ansehen“) is stored in the user's organization preferences 600 ms after the last change, one write per burst; a hard reload inside that window keeps the previous value, and a failed save keeps the local value for the session only.
