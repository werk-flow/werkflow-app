...
- Since `P1-07` (2026-08-07), open requests (`offen`/`in_klaerung`) surface as attention items on the shared `/aufgaben` surface for managers, showing the responsible person (`assigned_to`, now a visible ownership signal: „Mir zugewiesen" / „Zuständig: …" / „Nicht zugewiesen") and a derived age („offen seit N Tagen") — no new due-date storage. The recorded P1-10 boundary: manual follow-ups with owner and due time become their own object in `P1-10` and enter the attention pattern as a new item type; P1-07 deliberately did not create a competing follow-up concept. Promised-response commitments, qualification depth, response-time reporting, and request-level equipment references still wait for `P1-10`/`P1-18`. Converting a request into an update of *existing* work (rather than new work) is deferred to the service slices. Fixed in passing by the GG-02 cycle: the „Zuständig" selects in the request dialogs were silently empty since P1-02 (a PostgREST embed without a foreign-key path); assignment now works.
