...
- Request intake exists (P1-02), but promised-response commitments, follow-up ownership with due dates, qualification depth, response-time reporting, and request-level equipment references wait for `P1-07` (shared attention pattern), `P1-10` (relationship timeline), and `P1-18` (installed equipment). Converting a request into an update of *existing* work (rather than new work) is deferred to the service slices.
