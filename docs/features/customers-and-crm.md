...
- **Service-case conversion, P1-19:** a resolved request can instead be qualified exactly once into a reactive service case. Customer, contact, site, summary, details and urgency are carried forward, while the original request and its history remain the source record. The conversion does not create a job, schedule a visit or send a message. A request already converted to any target cannot be converted again.
...
- **Installed-equipment projection, P1-18:** each Einsatzort on the manager customer detail shows a compact list of recorded Anlagen & Geräte with number, category, identity and lifecycle state plus a link to the service-owned detail. An empty list means only that no equipment has been recorded. CRM neither owns nor edits the technical lifecycle, and site/customer access does not widen an employee beyond equipment explicitly linked to an exactly assigned job.
...
- P1-18 and P1-19 provide structured equipment and reactive-service links. Service contracts, recurring maintenance and a complete warranty history remain later scope.
