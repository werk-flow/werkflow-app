...
| [phase-1/pre-wave-3/05-beta-acceptance-and-production-rollout.md](plans/phase-1/pre-wave-3/05-beta-acceptance-and-production-rollout.md) | Living brief — beta acceptance on the final tree (fresh release verification and canary), the push to the preview for review, and the owner-triggered coordinated rollout to PROD and `main`, including the PROD document rule. Runs after steps 1 to 4 and only on the owner's request. |
