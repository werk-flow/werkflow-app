...
Status: living — last reviewed 2026-09-25; update this index when adding, moving, or retiring a doc
...
| [testing.md](technical/testing.md) | You implement or verify a change: plan selected groups, preserve clause coverage, qualify evidence, enforce deadlines, investigate failures, and keep the campaign inside its budget. |
...
| [test-incident-log.md](technical/test-incident-log.md)                   | A browser failure needs its evidence, classification and prevention tier recorded, or a campaign went over budget and the harness change is due. |
...
| [wave-2-audit.md](plans/phase-1/audits/wave-2-audit.md) | Living — accepted Wave 2 slice coverage and the remaining formal wave-end certification record; distinguishes later UI/UX verification evidence from the unrecorded wave-end gate. |
...
| [04-testing-system-restructure.md](plans/phase-1/hardening-2026-09/04-testing-system-restructure.md) | Closed implementation checkpoint of the 2026-09 rollout of test allocation, independent groups, selected verification and evidence reuse. History; the current execution model is decision 0007's 2026-09-25 amendment. |
...
| [phase-1/pre-wave-3/03-clock-fab-and-calendar-ux.md](plans/phase-1/pre-wave-3/03-clock-fab-and-calendar-ux.md) | Living — the pre-Wave-3 step 3 record: the clock-button flow inventory and redesign (action sheet, hot keys, resume after a break), the cropped-header diagnosis, the D3 and D5 freshness changes, the market research on time clocks and boards, the `P1-24a` Plantafel decisions and acceptance criteria; `audit:performance:calendar-live` handed its fresh pass to step 5 (closed 2026-09-18). |
...
| [0007-independent-test-groups.md](decisions/0007-independent-test-groups.md) | Current coverage allocation, independent execution, selected change acceptance, input-qualified release results, the execution model of 2026-09-25 (prepared runs, workers, chained tests, preference reset, Realtime probe, campaign budget) and the blocking rules. |
...
5. **Check**: `bun run docs:check` validates link resolution including heading fragments, index coverage, status shape, skill-mirror files in both directions, slice-record uniqueness, roadmap invariants, catalog ordering, and the two closing sections below. Semantic agreement with code still needs review. Run the check after any docs change.
