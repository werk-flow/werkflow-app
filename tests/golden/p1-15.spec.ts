...
test.describe('P1-15 structured site evidence @P1-15', () => {
  test('roles, immutable approval, structured measurement, lifecycle projection, and RLS', async ({
    ...
    await dialog
      ...
      .fill('Sicherheitsventil geprüft und Anlage entlüftet.');
    ...
    await expect
      .poll(async () => (await getWorkArtifactState(world.orgId, { jobNumber })).artifacts.length)
      .toBe(1);
    ...
    await expect(adminDialog.getByText('Intern freigegeben', { exact: false })).toBeVisible({
      timeout: 20_000,
    });
    ...
    expect(state.actions.map((action) => action.action_type)).toEqual([
      'review_requested',
      'internal_approved',
    ]);
    ...
    await selectOption(
      adminPage,
      measurementDialog.getByRole('combobox', { name: 'Aufmaßeinheit' }),
      'm'
    );
    await measurementDialog.getByLabel('Ort', { exact: true }).fill('Achse B');
    await measurementDialog
      .getByRole('button', { name: 'Als Entwurf speichern', exact: true })
      .click();
    await expect(measurementDialog.getByText(/Aufmaß · Entwurf · Version 1/)).toBeVisible({
      timeout: 20_000,
    });
    const measurementState = await getWorkArtifactState(world.orgId, {
      jobNumber,
    });
