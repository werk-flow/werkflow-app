...
export const RUN_ARCHIVE_ROOT = resolve(REPOSITORY_ROOT, '.agent-logs/playwright-runs');
...
export function updateRunManifest(
  ...
  return withFileLock(`${path}.lock`, () => {
    ...
    if (runKey === process.env.WERKFLOW_RUN_KEY) writeJsonAtomically(ACTIVE_MANIFEST_PATH, next);
    return next;
  });
}
...
export function clearActiveRunState(): void {
  ...
  for (const role of SESSION_ROLES) rmSync(storageStatePath(role), { force: true });
}
...
export function archiveActiveState(runKey = currentRunKey()): void {
  ...
  for (const source of [
    ...
    if (existsSync(source)) copyFileSync(source, resolve(target, basename(source)));
  }
}
...
export function restoreArchivedState(sourceRunKey: string): TestWorld {
  ...
  const sources = ['world.json', ...SESSION_ROLES.map((role) => `${role}.json`)].map(
    (fileName) => {
    const source = resolve(stateDirectory, fileName);
    if (!existsSync(source)) throw new Error(`Retained run ${sourceRunKey} is missing ${fileName}.`);
...
export function listRunManifests(): RunManifest[] {
  ...
  return readdirSync(RUN_ARCHIVE_ROOT, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && existsSync(manifestPath(entry.name)))
