...
import { createAndPublishWorkTemplate, createJob, createProject } from './support/steps'

test.describe.configure({ mode: 'serial' })

test.describe('P1-13 versioned work templates @P1-13', () => {
  test('a manager publishes an immutable job template with unified checklist metadata', async ({ adminPage, world }) => {
    ...
    expect(state.items.map((item) => item.content)).toEqual(['Anlage prüfen', 'Messwerte notieren'])
    ...
    expect(state.events.map((event) => event.event_type)).toEqual(expect.arrayContaining(['created', 'draft_saved', 'published']))
  })

  test('creation materializes editable existing primitives without stock or schedule side effects', async ({ adminPage, employeePage, world }) => {
    const templateName = `Wartung ${world.runId}`
    const jobNumber = `AUF-${world.runId}-P113-A`
    const employeeName = `${world.users.employee.firstName} ${world.users.employee.lastName}`
    await createJob(adminPage, { jobNumber, title: `Vorlagenauftrag ${world.runId}`, assignEmployeeName: employeeName, workTemplateName: templateName })
    ...
    await expect(employeePage.getByText('Anlage prüfen', { exact: true })).toBeVisible()
    await expect(employeePage.getByText('Nachweis erwartet: Foto der Messwerte', { exact: false })).toBeVisible()
    await employeePage.getByRole('button', { name: 'Punkt als erledigt markieren' }).first().click()
    await expect.poll(async () => (
      await getAppliedWorkTemplateState(world.orgId, { jobNumber })
    ).instructions[0].is_completed, { timeout: 20_000 }).toBe(true)
    await employeePage.reload()
    await expect(employeePage.getByRole('button', { name: 'Punkt als offen markieren' }).first()).toBeVisible()
  })

  test('a newer template version changes only future applications', async ({ adminPage, world }) => {
    const name = `Wartung ${world.runId}`
    ...
    await editor.getByLabel('Bezeichnung').first().fill('Anlage vollständig prüfen')
    ...
    const firstJob = await getAppliedWorkTemplateState(world.orgId, { jobNumber: `AUF-${world.runId}-P113-A` })
    expect(firstJob.instructions.map((item) => item.content)).toContain('Anlage prüfen')
    expect(firstJob.instructions.map((item) => item.content)).not.toContain('Anlage vollständig prüfen')

    const nextNumber = `AUF-${world.runId}-P113-B`
    await createJob(adminPage, { jobNumber: nextNumber, title: `Neue Version ${world.runId}`, workTemplateName: name })
    const nextJob = await getAppliedWorkTemplateState(world.orgId, { jobNumber: nextNumber })
    expect(nextJob.instructions.map((item) => item.content)).toContain('Anlage vollständig prüfen')
  })

  test('project templates create direct project planning and employees cannot manage templates', async ({ adminPage, employeePage, outsiderPage, world }) => {
    const name = `Sanierung ${world.runId}`
    await createAndPublishWorkTemplate(adminPage, { name, targetType: 'project', firstItem: 'Baustelle vorbereiten' })
    const projectNumber = `PRJ-${world.runId}-P113`
    await createProject(adminPage, { projectNumber, title: `Vorlagenprojekt ${world.runId}`, workTemplateName: name })
    const state = await getAppliedWorkTemplateState(world.orgId, { projectNumber })
    expect(state.applications).toHaveLength(1)
