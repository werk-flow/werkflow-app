...
async function completeManagerWork(
  ...
  const completeButton = lifecycle.getByRole('button', {
    name: 'Ausführung abgeschlossen', exact: true,
  });
  const startButton = lifecycle.getByRole('button', {
    name: 'In Ausführung', exact: true,
  });
  ...
  const overrideCheckbox = dialog.getByRole('checkbox', { name: /Manager-Ausnahme/ });
  if (override) {
  ...
  if (await reason.isVisible().catch(() => false)) {
    await reason.fill(
      'Ausführung wurde für die nachgelagerte Übergabeprüfung abgeschlossen.'
    );
  }
...
async function releaseHandover(page: Page, path: string): Promise<void> {
  ...
  await expect(section.getByText('Übergabepaket freigegeben', { exact: false }))
...
test.describe('P1-17 exhaustive office handover flows @AUDIT-W2-P1-17 @AUDIT-W2', () => {
  test('establishes job/project scope, exact routes, warning facts, and role boundaries', async ({
    adminPage, bueroPage, employeePage, outsiderPage, world,
  }) => {
    ...
    for (const [index, job] of [
      ...
      await createPlannedCalendarEntry(adminPage, {
        kind: 'job_visit', jobSearch: job.number, date: DATES[index + 1], time: '06:00',
    ...
    const before = await getWorkHandoverState(world.orgId, { jobNumber: fixture.firstJobNumber });
    await Promise.all([
      adminPage.goto(`/auftraege/projekt/${fixture.projectNumber}/${fixture.firstJobNumber}/uebergabe`),
      bueroPage.goto(`/auftraege/projekt/${fixture.projectNumber}/uebergabe`),
    ]);
    await expect(adminPage.getByTestId('work-handover-section')).toContainText('Noch kein Übergabepaket');
    await expect(adminPage.getByTestId('work-handover-section')).toContainText('Die Ausführung muss abgeschlossen sein');
    await expect(bueroPage.getByTestId('work-handover-section')).toBeVisible();
    expect(await getWorkHandoverState(world.orgId, { jobNumber: fixture.firstJobNumber })).toEqual(before);

    await employeePage.goto(`/auftraege/projekt/${fixture.projectNumber}/${fixture.firstJobNumber}/uebergabe`);
    ...
    for (const jobNumber of [fixture.firstJobNumber, fixture.secondJobNumber]) {
      ...
      if (state.package?.state !== 'released') {
        ...
        await uploadIntoDocumentsSection(
          adminPage, resolve(ARTIFACTS_DIR, 'upload-fixture.pdf'), 'upload-fixture'
        );
        await adminPage.goto('/aufgaben');
        await expect(adminPage.getByTestId('aufgaben-content')).toHaveAttribute('data-loaded', 'true');
        await completeManagerWork(
          bueroPage,
          `/auftraege/projekt/${fixture.projectNumber}/${jobNumber}`
        );
        await expect(adminPage.getByTestId('attention-work-handover-tasks'))
          .toContainText(jobNumber, { timeout: 30_000 });
        await releaseHandover(
          bueroPage,
          `/auftraege/projekt/${fixture.projectNumber}/${jobNumber}/uebergabe`
        );
        await expect(adminPage.getByTestId('attention-work-handover-tasks').getByText(jobNumber))
      ...
      expect(releasedItems.map((item) => ({
        sourceKind: item.source_kind,
        documentId: item.document_id,
        documentVersionNumber: item.document_version_number,
      }))).toEqual(state.draftItems.map((item) => ({
        sourceKind: item.source_kind,
        documentId: item.document_id,
        documentVersionNumber: item.document_version_number,
      })));
      expect(releasedItems.length).toBeGreaterThan(0);
      expect(releasedItems.every((item) => (
        item.source_kind === 'document_version' && item.document_version_number === 1
      ))).toBe(true);
  ...
  test('composes a project from immutable child releases and rejects a stale office draft', async ({
    ...
    await expect(adminSection.getByText('Entwurf gespeichert.')).toBeVisible({ timeout: 20_000 });
    ...
    await expect(refreshed.getByText('Übergabepaket freigegeben', { exact: false }))
      .toBeVisible({ timeout: 30_000 });
    await preview.close();
    ...
    expect(state.releases[0]).toMatchObject({ commercial_readiness: 'ready_for_commercial_review' });
    expect(state.target).toMatchObject({ execution_state_override: 'handed_over' });
  });
  ...
  test('preserves predecessor releases through withdrawal, correction, and successor release', async ({
    ...
    if (initialState.package?.state !== 'reopened') {
      ...
      await expect(section.getByText('Übergabe zurückgenommen.', { exact: false }))
        .toBeVisible({ timeout: 20_000 });
      await adminPage.reload();
      section = adminPage.getByTestId('work-handover-section');
      await section.getByLabel('Ausführung erneut öffnen')
      ...
      await expect(section.getByText('Ausführung zur Korrektur geöffnet.'))
        .toBeVisible({ timeout: 20_000 });
    }
    ...
    expect(state.events.map((event) => event.event_type)).toEqual(expect.arrayContaining([
      'handover_withdrawn', 'review_returned', 'execution_reopened', 'successor_created',
    ]));
    const lifecycle = await getWorkLifecycleState(world.orgId, {
      projectNumber: fixture.projectNumber,
    });
    expect(lifecycle.executionEvents.map((event) => event.event_type)).toEqual(expect.arrayContaining([
      'handed_over', 'handover_withdrawn', 'reopened', 'handed_over',
    ]));
  });
});
