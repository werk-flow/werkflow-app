...
const DATES = Array.from({ length: 5 }, (_, index) => shiftIsoDate(berlinTodayIso(), 70 + index))

test.describe('P1-13 exhaustive work-template flows @AUDIT-W2-P1-13 @AUDIT-W2', () => {
  test('empty state, role denial, validation, creation, filters, and safe realtime catch-up', async ({ adminPage, bueroPage, employeePage, outsiderPage, world }) => {
    // P1-13-F01, F02, F03, F09, F26, F27.
    await outsiderPage.goto('/arbeitsvorlagen')
    await expect(outsiderPage.getByText('Erste Arbeitsvorlage anlegen', { exact: true })).toBeVisible()
    await expect(outsiderPage.getByText('Heizungswartung Standard', { exact: true })).toHaveCount(0)
    await employeePage.goto('/arbeitsvorlagen')
    await expect(employeePage).toHaveURL(/\/dashboard/)

    await bueroPage.goto('/arbeitsvorlagen')
    await bueroPage.getByRole('button', { name: /Vorlage erstellen|Erste Vorlage erstellen/ }).first().click()
    await expect(bueroPage.getByRole('dialog')).toBeVisible()

    await adminPage.goto('/arbeitsvorlagen')
    await adminPage.getByRole('button', { name: /Vorlage erstellen|Erste Vorlage erstellen/ }).first().click()
    const dialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Arbeitsvorlage erstellen' }) })
    ...
    await dialog.getByRole('button', { name: 'Erstellen', exact: true }).click()
    const editor = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: /Entwurf · Version 1/ }) })
    await expect(editor.getByRole('heading', { name: /Entwurf · Version 1/ })).toBeVisible({ timeout: 20_000 })
    await editor.getByRole('button', { name: 'Veröffentlichen', exact: true }).click()
    await expect(editor.getByText('Füge mindestens eine Aufgabe oder einen Checklistenpunkt hinzu.')).toBeVisible()
    ...
    await expect(bueroPage.getByText(`Audit Wartung ${world.runId}`, { exact: true })).toBeVisible({ timeout: 20_000 })
    await bueroPage.locator('#template-search').fill(`Wiederkehrende Projektarbeit ${world.runId}`)
    await expect(bueroPage.getByText(`Audit Wartung ${world.runId}`, { exact: true })).toBeVisible()
    await bueroPage.locator('[aria-label="Ziel filtern"]').click()
    await bueroPage.getByRole('option', { name: 'Nur Projekte' }).click()
    await bueroPage.locator('[aria-label="Status filtern"]').click()
    await bueroPage.getByRole('option', { name: 'Entwürfe' }).click()
    await expect(bueroPage.getByText(`Audit Wartung ${world.runId}`, { exact: true })).toBeVisible()
    await bueroPage.locator('#template-search').fill('nicht vorhanden')
    await expect(bueroPage.getByText('Keine Arbeitsvorlage passt zu Suche und Filtern.')).toBeVisible()
  })

  test('draft content covers tasks, evidence, material, qualifications, dependencies, and immutable publish', async ({ adminPage, world }) => {
    // P1-13-F04, F05, F06, F07, F08, F10.
    const name = `Audit Komplett ${world.runId}`
    await createAndPublishWorkTemplate(adminPage, { name, targetType: 'job', firstItem: 'Sicherheitsprüfung', secondItem: 'Messung dokumentieren', evidenceDescription: 'Foto des Messgeräts' })
    await adminPage.goto('/arbeitsvorlagen')
    await adminPage.locator('#template-search').fill(name)
    await adminPage.getByRole('button', { name: 'Öffnen', exact: true }).click()
    let editor = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: `${name} · Version 1`, exact: true }) })
    await editor.getByRole('button', { name: 'Neue Version' }).click()
    await expect(editor).toHaveCount(0, { timeout: 15_000 })
    await adminPage.getByRole('button', { name: 'Öffnen', exact: true }).click()
    editor = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: /Entwurf · Version 2/ }) })

    let labels = editor.getByLabel('Bezeichnung')
    const safetyCard = labels.nth(0).locator('xpath=ancestor::*[@data-slot="card"][1]')
    ...
    labels = editor.getByLabel('Bezeichnung')
    await labels.last().fill('Temporärer Punkt')
    await labels.last().locator('xpath=ancestor::*[@data-slot="card"][1]').getByRole('button', { name: 'Eintrag löschen' }).click()
    await expect(editor.getByLabel('Bezeichnung')).toHaveCount(2)
    await safetyCard.getByRole('button', { name: 'Eintrag nach unten' }).click()

    const safetyDependency = editor.getByRole('combobox', { name: 'Voraussetzungen für Sicherheitsprüfung' })
    ...
    await toggleInSearchableMulti(adminPage, safetyDependency, ['Messung dokumentieren'])
    await editor.getByLabel('Dokumentkategorie').first().click()
    ...
    const materialCard = editor.locator('[data-slot="card"]').filter({ hasText: 'Artikel' }).last()
    await materialCard.getByRole('combobox').first().click()
    await adminPage.getByRole('button', { name: 'Neuen Artikel erstellen' }).click()
    const itemDialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Artikel erstellen' }) })
    ...
    await materialCard.getByRole('combobox').nth(1).click()
    await adminPage.getByRole('button', { name: 'Neues Lager erstellen' }).click()
    const locationDialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Lager erstellen' }) })
    ...
    const qualificationRow = editor.locator('section').filter({ hasText: 'Geplante Qualifikationen' }).locator('.rounded-lg.border').last()
    await qualificationRow.getByRole('combobox').click()
    await adminPage.getByRole('button', { name: 'Neue Qualifikation erstellen' }).click()
    const capabilityDialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Qualifikation erstellen' }) })
    ...
    await expect(adminPage.getByText('Entwurf gespeichert.', { exact: true })).toBeVisible({ timeout: 20_000 })
    await editor.getByRole('button', { name: 'Veröffentlichen' }).click()
    ...
    expect(latestItems[1]).toMatchObject({ item_kind: 'checklist', group_label: 'Inbetriebnahme', notes: 'Vor Ort mit dem Kunden abstimmen.' })
    expect(state.materials[0]).toMatchObject({ planned_quantity: 3, is_billable: false, notes: 'Nur für die Einsatzplanung.' })
    expect(state.materials[0].preferred_location_id).not.toBeNull()
    expect(state.capabilities[0].require_confirmation).toBe(true)
  })

  test('version history, archive/reactivation, picker application on create, and snapshot meaning persist', async ({ adminPage, employeePage, world }) => {
    // P1-13-F11, F12, F13, F14, F19, F20, F21, F22.
    const name = `Audit Komplett ${world.runId}`
    const jobNumber = `AUF-${world.runId}-P113-70`
    const employeeName = `${world.users.employee.firstName} ${world.users.employee.lastName}`
    await createJob(adminPage, { jobNumber, title: `Audit Vorlage ${world.runId}`, assignEmployeeName: employeeName, plannedDateDigits: digits(DATES[0]), workTemplateName: name, qualificationOverrideReason: 'Abweichung für den vollständigen Auditfluss.' })
    ...
    expect(state.qualificationAssessments[0].override_reason).toBe('Abweichung für den vollständigen Auditfluss.')
    expect(state.qualificationAssessments[0].coverage_fingerprint).toBeTruthy()

    await employeePage.goto(`/auftraege/${jobNumber}`)
    await expect(employeePage.getByText('Sicherheitsprüfung', { exact: true })).toBeVisible()
    await expect(employeePage.getByText('Messung dokumentieren', { exact: true })).toBeVisible()
    await expect(employeePage.getByText(/Aufgabe · Optional/)).toBeVisible()
    await expect(employeePage.getByText('Voraussetzung: Sicherheitsprüfung', { exact: true })).toBeVisible()
    await expect(employeePage.getByText(/Nachweis erwartet:/).first()).toBeVisible()
    const safetyInstruction = employeePage.getByText('Sicherheitsprüfung', { exact: true }).locator('xpath=ancestor::div[.//button[contains(@aria-label,"Punkt als")]][1]')
    await safetyInstruction.getByRole('button', { name: 'Punkt als erledigt markieren' }).click()
    await expect.poll(async () => (await getAppliedWorkTemplateState(world.orgId, { jobNumber })).instructions.find((item) => item.content === 'Sicherheitsprüfung')?.is_completed, { timeout: 20_000 }).toBe(true)
    await employeePage.reload()
    await expect(employeePage.getByText('Sicherheitsprüfung', { exact: true }).locator('xpath=ancestor::div[.//button[contains(@aria-label,"Punkt als")]][1]').getByRole('button', { name: 'Punkt als offen markieren' })).toBeVisible()
    state = await getAppliedWorkTemplateState(world.orgId, { jobNumber })
    const completedInstruction = state.instructions.find((item) => item.content === 'Sicherheitsprüfung')
    ...
    await expect(adminPage.getByRole('heading', { name: `Audit Vorlage ${world.runId}` })).toBeVisible({ timeout: 20_000 })
    const detailButtons = adminPage.getByRole('button', { name: 'Eintragsdetails bearbeiten' })
    await expect(detailButtons).toHaveCount(2)
    const detailsDialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Eintragsdetails bearbeiten' }) })
    await expect(async () => {
      if (!(await detailsDialog.isVisible().catch(() => false))) await detailButtons.last().click()
    ...
    await expect(adminPage.getByText(/Vor Ort geändert/)).toBeVisible()
    state = await getAppliedWorkTemplateState(world.orgId, { jobNumber })
    const editedInstruction = state.instructions.find((item) => item.content === 'Sicherheitsprüfung')!
    ...
    expect(state.evidence[0]).toMatchObject({ description: 'Foto direkt am Auftrag', document_category: 'other' })
    ...
    const materialDialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Materialposition bearbeiten' }) })
    ...
    await adminPage.getByRole('button', { name: `${`Gasprüfung ${world.runId}`} als Anforderung entfernen` }).click()
    await expect.poll(async () => (await getAppliedWorkTemplateState(world.orgId, { jobNumber })).capabilities.length, { timeout: 20_000 }).toBe(0)
    state = await getAppliedWorkTemplateState(world.orgId, { jobNumber })
    expect(state.materials[0]).toMatchObject({ planned_quantity: 4, notes: 'Am Auftrag angepasst.' })
    ...
    await templateEditor.getByLabel('Bezeichnung').first().fill('Messung in neuer Vorlage')
    await templateEditor.getByRole('button', { name: 'Veröffentlichen' }).click()
    await expect(templateEditor).toHaveCount(0, { timeout: 20_000 })
    await adminPage.goto(`/auftraege/${jobNumber}`)
    await expect(adminPage.getByText('Messung dokumentieren', { exact: true })).toBeVisible()
    await expect(adminPage.getByText('Messung in neuer Vorlage', { exact: true })).toHaveCount(0)
    ...
    ).template.archived_at, { timeout: 20_000 }).not.toBeNull()
    ...
    await adminPage.locator('#work-template-job').click()
    await expect(adminPage.getByRole('listbox').getByText(name, { exact: true })).toHaveCount(0)
    await adminPage.keyboard.press('Escape')
    await adminPage.getByRole('dialog').getByRole('button', { name: 'Schließen', exact: true }).click()
    await adminPage.goto('/arbeitsvorlagen')
    await adminPage.locator('#template-search').fill(name)
    await adminPage.locator('[aria-label="Status filtern"]').click()
    ...
    expect(appliedEvent.event_payload).toMatchObject({ jobId: state.targetId, versionNumber: 2 })
    ...
    await adminPage.getByRole('button', { name: 'Öffnen', exact: true }).click()
    await expect(adminPage.getByRole('dialog').getByText(`${jobNumber} · Audit Vorlage ${world.runId}`, { exact: true })).toBeVisible()
  })

  test('after-creation preview, duplicate/additional warnings, and project-direct rows are persisted', async ({ adminPage, bueroPage, world }) => {
    test.slow()
    // P1-13-F17, F18, F23, F24.
    const jobNumber = `AUF-${world.runId}-P113-71`
    await createJob(adminPage, { jobNumber, title: `Nachträglich ${world.runId}`, plannedDateDigits: digits(DATES[1]) })
    let state = await getAppliedWorkTemplateState(world.orgId, { jobNumber })
    ...
    const dialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Arbeitsvorlage anwenden' }) })
    await selectFromSearchable(adminPage, dialog.getByRole('combobox'), `Audit Komplett ${world.runId}`)
    ...
    await expect(bueroPage.getByText('Messung in neuer Vorlage', { exact: true })).toBeVisible({ timeout: 25_000 })

    await adminPage.getByRole('button', { name: 'Vorlage anwenden' }).click()
    const duplicateDialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Arbeitsvorlage anwenden' }) })
    await selectFromSearchable(adminPage, duplicateDialog.getByRole('combobox'), `Audit Komplett ${world.runId}`)
    await expect(duplicateDialog.getByText('Diese Version wurde bereits angewendet.')).toBeVisible()
    await expect(duplicateDialog.getByRole('button', { name: 'Anwenden', exact: true })).toBeDisabled()
    await duplicateDialog.getByRole('button', { name: 'Abbrechen' }).click()

    const additionalTemplate = `Audit Zusatz ${world.runId}`
    await createAndPublishWorkTemplate(adminPage, { name: additionalTemplate, targetType: 'job', firstItem: 'Zusätzliche Sichtprüfung' })
    await adminPage.goto(`/auftraege/${jobNumber}`)
    await adminPage.getByRole('button', { name: 'Vorlage anwenden' }).click()
    const additionalDialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Arbeitsvorlage anwenden' }) })
    await selectFromSearchable(adminPage, additionalDialog.getByRole('combobox'), additionalTemplate)
    await expect(additionalDialog.getByText('Weitere Vorlage ergänzen. Vorhandene Planung bleibt bestehen.')).toBeVisible()
    await expect(additionalDialog.getByRole('button', { name: 'Anwenden', exact: true })).toBeDisabled()
    ...
    await createAndPublishWorkTemplate(adminPage, { name: projectTemplate, targetType: 'project', firstItem: 'Projektstart dokumentieren' })
    await adminPage.goto('/arbeitsvorlagen')
    await adminPage.locator('#template-search').fill(projectTemplate)
    await adminPage.getByRole('button', { name: 'Öffnen', exact: true }).click()
    ...
    await selectFromSearchable(adminPage, projectEditor.locator('[data-slot="card"]').filter({ hasText: 'Artikel' }).last().getByRole('combobox').first(), `Dichtung ${world.runId}`)
    await projectEditor.getByRole('button', { name: 'Qualifikation', exact: true }).click()
    await selectFromSearchable(adminPage, projectEditor.locator('section').filter({ hasText: 'Geplante Qualifikationen' }).locator('.rounded-lg.border').last().getByRole('combobox'), `Gasprüfung ${world.runId}`)
    ...
    await createProject(adminPage, { projectNumber, title: `Audit Projekt ${world.runId}`, workTemplateName: projectTemplate })
    ...
    await createJob(adminPage, { jobNumber: childJobNumber, title: `Späterer Unterauftrag ${world.runId}`, projectNumber })
  ...
  test('request conversion applies atomically and every shared creation context exposes the optional picker', async ({ adminPage, world }) => {
    test.slow()
    // P1-13-F15, F16.
    ...
    await createRequestViaDialog(adminPage, { summary: `Vorlagenanfrage ${world.runId}`, requestNumber, clientName: customerName })
    await convertRequestToJobViaDialog(adminPage, { workTemplateName: `Audit Komplett ${world.runId}`, plannedDate: DATES[3] })
    ...
    expect(await getWorkTemplateApplicationCountForTarget(world.orgId, { jobId: conversion.convertedJobId! })).toBe(1)

    await adminPage.goto('/kunden')
    await adminPage.getByRole('row').filter({ hasText: customerName }).click()
    const customerCreateButton = adminPage.getByRole('button', { name: 'Erstellen', exact: true })
    await expect(customerCreateButton).toBeVisible({ timeout: 15_000 })
    await customerCreateButton.click()
    await adminPage.getByRole('tab', { name: 'Auftrag erstellen' }).click()
    await expect(adminPage.locator('#work-template-job')).toBeVisible({ timeout: 15_000 })
    await adminPage.getByRole('dialog').getByRole('button', { name: 'Schließen', exact: true }).click()
    ...
    const employeeCreateButton = adminPage.getByRole('button', { name: 'Auftrag erstellen' })
    await expect(employeeCreateButton).toBeVisible({ timeout: 15_000 })
    await employeeCreateButton.click()
    await expect(adminPage.locator('#work-template-job')).toBeVisible({ timeout: 15_000 })
    await adminPage.getByRole('dialog').getByRole('button', { name: 'Schließen', exact: true }).click()

    await adminPage.goto(`/auftraege/projekt/PRJ-${world.runId}-P113-72`)
    const projectCreateButton = adminPage.getByRole('button', { name: 'Auftrag hinzufügen' })
    ...
    const calendarCreateButton = adminPage.getByRole('button', { name: /Kalendereintrag|Eintrag erstellen|Neuer Eintrag/ }).first()
    await expect(calendarCreateButton).toBeVisible({ timeout: 15_000 })
    await calendarCreateButton.click()
    const calendarDialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Kalendereintrag erstellen' }) })
    await calendarDialog.getByRole('tab', { name: 'Auftrag erstellen' }).click()
    await expect(calendarDialog.locator('#work-template-job')).toBeVisible({ timeout: 15_000 })
    await calendarDialog.getByRole('button', { name: 'Schließen', exact: true }).click()
  })

  test('retired references fail atomically while role and organization boundaries stay closed', async ({ adminPage, employeePage, outsiderPage, world }) => {
    test.slow()
    ...
    await expect(outsiderPage.getByText('Noch keine passende Vorlage veröffentlicht.')).toBeVisible({ timeout: 15_000 })
    await expect(outsiderPage.getByRole('link', { name: 'Arbeitsvorlagen verwalten' })).toHaveAttribute('href', '/arbeitsvorlagen')
    await expect(outsiderPage.getByText(`Audit Komplett ${world.runId}`, { exact: true })).toHaveCount(0)
    ...
    const capabilityRow = adminPage.getByTestId('capability-definition-row').filter({ hasText: `Gasprüfung ${world.runId}` })
    await expect(capabilityRow).toBeVisible({ timeout: 15_000 })
    await capabilityRow.getByRole('button', { name: 'Archivieren' }).click()
    await expect(adminPage.getByText('Der Begriff wurde archiviert.')).toBeVisible({ timeout: 20_000 })

    const customerName = `Audit Kunde ${world.runId}`
    const failedRequestNumber = `ANF-${world.runId}-P113-F`
    await createRequestViaDialog(adminPage, { summary: `Fehlerhafte Vorlagenanfrage ${world.runId}`, requestNumber: failedRequestNumber, clientName: customerName })
    await adminPage.getByRole('button', { name: 'Umwandeln' }).click()
    const conversionDialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Anfrage umwandeln' }) })
    await selectFromSearchable(adminPage, conversionDialog.locator('#work-template-job'), `Audit Komplett ${world.runId}`)
    await expect(conversionDialog.locator('#convert-number')).toHaveValue(/.+/, { timeout: 15_000 })
    ...
    await expect(conversionDialog.getByText('Die Arbeitsvorlage verweist auf nicht mehr aktive Stammdaten.')).toBeVisible({ timeout: 20_000 })
    ...
    await conversionDialog.locator('form').getByRole('button', { name: 'Abbrechen', exact: true }).click()

    const cleanJobNumber = `AUF-${world.runId}-P113-74`
    await createJob(adminPage, { jobNumber: cleanJobNumber, title: `Referenzprüfung ${world.runId}`, plannedDateDigits: digits(DATES[4]) })
    await adminPage.goto(`/auftraege/${cleanJobNumber}`)
    await adminPage.getByRole('button', { name: 'Vorlage anwenden' }).click()
    const applyDialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Arbeitsvorlage anwenden' }) })
    await selectFromSearchable(adminPage, applyDialog.getByRole('combobox'), `Audit Komplett ${world.runId}`)
    await applyDialog.getByRole('button', { name: 'Anwenden', exact: true }).click()
    await expect(applyDialog.getByText(`„Gasprüfung ${world.runId}“ ist nicht mehr aktiv. Korrigiere die Vorlage und versuche es erneut.`)).toBeVisible({ timeout: 20_000 })
    let cleanState = await getAppliedWorkTemplateState(world.orgId, { jobNumber: cleanJobNumber })
    ...
    await adminPage.getByRole('button', { name: 'Vorlage anwenden' }).click()
    const retryDialog = adminPage.getByRole('dialog').filter({ has: adminPage.getByRole('heading', { name: 'Arbeitsvorlage anwenden' }) })
    await selectFromSearchable(adminPage, retryDialog.getByRole('combobox'), `Audit Zusatz ${world.runId}`)
    await retryDialog.getByRole('button', { name: 'Anwenden', exact: true }).click()
    await expect(retryDialog).toHaveCount(0, { timeout: 20_000 })
    cleanState = await getAppliedWorkTemplateState(world.orgId, { jobNumber: cleanJobNumber })
    expect(cleanState.applications).toHaveLength(1)
    expect(cleanState.instructions.map((item) => item.content)).toEqual(['Zusätzliche Sichtprüfung'])
  })
})
